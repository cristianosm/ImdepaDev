...
±±∫Desc.     ≥Foco Marca                                                  ∫±±
±±∫          ≥                                                            ∫±±
±±ÃÕÕÕÕÕÕÕÕÕÕÿÕÕÕÕÕÕÕÕÕÕ—ÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕπ±±
±±∫  DATA    ≥ ANALISTA ≥ ALTERACOES                                      ∫±±
±±ÃÕÕÕÕÕÕÕÕÕÕÿÕÕÕÕÕÕÕÕÕÕÿÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕπ±±
±±∫          ≥          ≥                                                 ∫±±
±±∫          ≥          ≥                                                 ∫±±
±±ÃÕÕÕÕÕÕÕÕÕÕÿÕÕÕÕÕÕÕÕÕÕœÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕπ±±
±±∫Uso       ≥ Imdepa                                                     ∫±±
...
Private aDatas  	:= {}
Private aDiasUt 	:= {}
Private nDiasuMA	:= 0 //Dias uteis total do mÍs atual
Private aTotalMes 	:= {}
Private aTotalGeral := {}
Private lWorkFlow   := .F.
Private NCOUNT      :=  1
...
If Type("dDatabase") == "U"
	...
	//Inserido por Edivaldo GonÁalves Cordeiro em 30/06/2007
	cEmp := '01'
	cFil := '05'
...
NEXT
//cTexto+= "  Meta  | Meta %  | MÍs Ant.%| MMAA % |  12Meses % |   Peso %  |"		//        cTexto+= "  Meta     | MÍs Ant.%| MMAA % | 12Meses% "
...
//CQUERY += "	         SUM (D2_PRCVEN * (D2_QUANT - D2_QTDEDEV) ) AS VLR_VENDA  " // Considerando a devoluÁ„o diretamente na  venda
//CQUERY += "	         SUM (D2_PRCVEN * (D2_QUANT ) ) AS VLR_VENDA  " // Considerando a devoluÁ„o diretamente na  venda
...
//Inserido por Edivaldo GonÁalves Cordeiro em 24/11/02
...
For k:=1 to Len(aDados)
	ctitulo   := 'Foco Marca - ' + cTipoVen + ': ' + aDados[k][1] + '- Data da GeraÁ„o: '+ DTOC(dDatabase) //"RELATORIO FOCO MARCA - "+cTipo+" - PERIODO: "+ DTOC(dUltDia) +" - "+ DTOC(dDataConsiderada)
...
//	@ nLin,02 PSAY SubStr( Tabela( "Z8", aDados[k][2] ), 1, 25 )
	...
	/*
	// ----------R$ SOMATARIO MÍS ATUAL + OS ULTIMO 11 MESES / 12  ----------

	nVlr12M :=  nTotal12Mes / 12
	@ nLin,nCol+33 PSAY	PADL(TRANSFORM(nVlr12M,'@E 999,999'),07," ")

	*/
	// ---------- SOMATARIO MÍS ATUAL + OS ULTIMO 11 MESES / 12 , EM % ----------
	...
	**********************************************************************
	...
	If Mod( K, Len(aMarcas) ) == 0
		...
		*/
		...
		nLin++
		@ nLin,00 PSAY "Data de GeraÁ„o : " + DTOC(dDatabase)+ " "
		nLin++
		...
		IF lWorkFlow == .T.
			...
			If !ExistBlock("MSRELTOHTML")
				CONOUT("FOCO MARCA - FunÁ„o MSRELTOHTML n„o compilada. N„o ir· gerar HTML para envio")
			Endif
...
CMENSAGEM 	:= "RELATÛRIO FOCO MARCA"
CSUBJECT 	:= "FOCO MARCA "
NI        	:= 1
N 			:= 1
...
IF LRESULT .AND. LAUTH
	...
	IF !LRESULT
		...
		CONOUT("FMARCA - ERRO DE AUTENTICACAO DE E-MAIL NO JOB DE AÁAO DE VENDAS "+CERROR)
		RETURN NIL
	ENDIF
ELSE
	IF !LRESULT
...
IF lTeste
	...
	SUBJECT 'Foco Marca  - ' + cTipoVen + ': ' + aDados[k][1] + " - "+AllTrim(Posicione("SA3",1,xFilial("SA3")+ aDados[k][1],"A3_NOME"))+ ' - Data da GeraÁ„o: '+ DTOC(dDatabase)  ; //SUBJECT 'RelatÛrio Foco Marca  - Gerado: '+ DTOC(dDatabase)  ;
	BODY CHTML ;
...
IF ! SX1->(DBSEEK(CPERG,.F.))
	...
	AADD(AHELP03, "ATÈ GERENTE: ?" 			)
	AADD(AHELP04, "DO GESTOR DE CONTAS: ?"	)
	AADD(AHELP05, "ATÈ GESTOR DE CONTAS: ?" )
	AADD(AHELP06, "DO VEND.INTERNO: ?" 		)
	AADD(AHELP07, "ATÈ VEND.INTERNO: ?" 	)
	...
	PUTSX1( "RFOCOM","03", "ATÈ GERENTE:", "", "","MV_CH3","C",06,0,0,"G","","SA3",;
	...
	PUTSX1( "RFOCOM","05", "ATÈ GESTOR DE CONTAS:", "", "","MV_CH5","C",06,0,0,"G","","SA3",;
	...
	PUTSX1( "RFOCOM","07", "ATÈ VEND.INTERNO:", "", "","MV_CH7","C",06,0,0,"G","","SA3",;
...
Default aCustomText := Nil // Par‚metro que se passado suprime o texto padrao desta funÁ„o por outro customizado
...
@(++nLin), 0 PSAY "*"+INIFIELD+"IMDEPA - RelatÛrio Foco Marca via Workflow"+FIMFIELD
// Folha
If __SetCentury()
	nRecuo := 20
else
...
If aCustomText == Nil // Imprime o cabeÁalho padr„o dos relatÛrios
