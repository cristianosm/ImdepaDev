...
***********************************************************************
	...
	Local lPrcHist	:=	.F.
	Local nValorTrans := 0 // Valor que deve ser utilizado em caso de transferencias...
...
//| Tratamento para Transferencias entre Filiais Imdepa....
//| Nestes casos nao deve pegar o valor de alguma Lista de Preco, ele deve manter valor ja calculado..no IMDA090
//| Data: 03/07/15
	If ehTransf( @nValorTrans , cProduto, nTipo)
	
		Conout('ehtransf : '+ cValtoChar(nValorTrans) + ' cProduto = ' + cProduto + ' nTipo = ' +  cValtoChar(nTipo) )
...
Return()
*******************************************************************************
Static Function ehTransf(nValorTrans, cProduto, nTipo)
*******************************************************************************
	Local lRetorno 	:= .F.
	Local nPosAr	:= 0
	Local nTamAr	:= 0
...
//| Verifica se Existe Array ? 
	If Type('aIPvImda090') <> 'U'
		...
		If ValType ( aIPvImda090 ) == 'A' //| Existe o Array ?
	
			nTamAr := Len (aIPvImda090)
			If  nTamAr > 0 // Verifica se o array eh maio que ZERO
		 
		// Pesquisa o Produto
				nPosAr := ASCAN( aIPvImda090,{ |X| cProduto == X[2]} )
		 
		// Captura o Valor ja Calculado no Fonte IMDA090
				nValorTrans := 	aIPvImda090[nPosAr][6]
		
		//|	Tipo (1=Preço/2 = Fator de acréscimo ou desconto)
				IF nTipo == 2 // Fator
					nValorTrans	:=	0		//|	Tipo (1=Preço/2 = Fator de acréscimo ou desconto)
				EndIf
		//Apaga o Produto do Array
		//ADEL ( aIPvImda090, nPosAr )
		
		// Redefine o tamanho do Array ou o elimina
		//If ( (nTamAr - 1) <= 0 )
		//	aIPvImda090 := Nil
		//Else
		//	ASIZE (  aIPvImda090, nTamAr - 1 )
		//EndIF
