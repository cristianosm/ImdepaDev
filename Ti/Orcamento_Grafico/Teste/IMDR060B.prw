...
#Define FNISPAG			39		//| Numero de Itens que podem ser impressos apartir da Segunda Pagina
...
/*****************************************************************************\
**---------------------------------------------------------------------------**
** FUNÇÃO   : IMDR060B    | AUTOR : Cristiano Machado  | DATA : 18/02/2016   **
**---------------------------------------------------------------------------**
** DESCRIÇÃO: Orçamento de Vendas, Utilizado na Tela da Verdade              **
**---------------------------------------------------------------------------**
...
*******************************************************************************
	...
	bValida := {|| lContinua, lContinua := Valida()  }  //| Bloco com Validaçoes Necessárias...
...
*******************************************************************************
	...
	//| Somente Substitui o __ACOLS caso seja Orçamento já Faturado... |
	If lFaturado
		__aCCols := aCols
	EndIf
...
Return()
*******************************************************************************
Static Function  Valida()//| Validações Necessarias
*******************************************************************************
	Local lValidacao := .T.

	//| Solicita o Click na Condição de Pagamento antes de emitir o Orçamento...

	If !ExistDir("C:\MP11")
		If MakeDir ("C:\MP11" ) <> 0
			IW_MsgBox("Não foi possivel criar o Diretório 'C:\MP11' este é necessário, para salvar o arquivo PDF. Entrar em contato com a TI.","Impressao de Orçamento/Pedido", "ALERT")
			lValidacao := _SAIR_
		EndIf
	EndIf
	...
	IF !_ljaCPaCP .And. lValidacao
		IW_MsgBox("Por favor, clique na Condição de pagamento antes de imprimir o Orçamento","Impressao de Orçamento/Pedido", "ALERT")
		lValidacao := _SAIR_
	EndIf
	...
	If lFaturado == NAOFATURANDO //| Somente Efetua esta validacao em Orçamento não Faturados... |
		If lValidacao .And. (dDataBase >  M->UA_DTLIM .AND. M->UA_DOC<>' ') .And. !lPerg
			IW_MsgBox("Como este Orçamento esta Vencido, realize a gravacao antes da Impressao","Impressao de Orçamento/Pedido", "ALERT")
			lValidacao := _SAIR_
		EndIf
	EndIf
...
*******************************************************************************
	...
	For nT := 1 To Len(aItens)
		...
		DbSeek(xFilial("SB1")+aItens[nT][I_CODIGO], .F. )
...
*******************************************************************************
	...
	//| Mensagem Linha 01
	aMens[M_LINHA1] := '* Faturamento Mínimo é de R$ ' + Alltrim(Transform( GetMv("IM_PEDMINF"),'@E 999,999.99')) +' Reais'
	...
	If SA1->A1_EST == 'GO' .And. SA1->A1_GRPSEG $ "AG3/AU3/IN3" .And. cFilant == '04'
		aMens[M_LINHA3] := '* Ao comparar nossos precos com outros fornecedores locais, favor verificar se a Subs. Trib. esta destacada (TARE).'
	EndIf
			...
			If !U_FCnaeAuto()// Verifica Cnae Do Cliente
				If SB1->B1_INDUSTR == "S"
					If (! cFilAnt $ Alltrim( GetMv("MV_MENFIL"))) .And. (! cFilAnt + SA1->A1_EST $ Alltrim( GetMv("MV_MENFIL1"))) .And. SA1->A1_CONAGRI <> "1"  //Agostinho - 24/03/2009 - Chamado AAZOSN retirada da mensagem para as filias 02-CUI, 04-GOI e 05-CDP quando venda para o RS
						aMens[M_LINHA4] := '* Mercadorias P/ Uso e Aplicacao Exclusivamente Industrial Nao Sujeita a ST Cfe Protocolo 49/08 '
					EndIf
				EndIf
			EndIf
		EndIf
	...
	//| Mensagem RODAPE
	aMens[M_RODAPE] := Padc('[ Proposta Válida por 2 Dias - Será Acrescida Despesa Bancária por Duplicata ]',120)

	
Return()
*******************************************************************************
Static Function DMarketing(aItem) //| Verifica e Monta as Mensagens de Marketing
*******************************************************************************
...
*******************************************************************************
	...
	For nPagina := 1 To nTotPag
		oProcess:IncRegua2("Pagina..." + cValToChar(nPagina))
...
*******************************************************************************
	...
	//| NCM
	oFont :=  TFont():New( 'Courier new' , , -9);	oFont:Italic 	:= .T.
	oPrn:Say( eVal(bPon) , 350+nVIC , aItem[I_CODNCM] , oFont ,   , 0 , )
...
*******************************************************************************
	...
	If lMsgMark //| Verifica se deve imprimir a Mensagem de Marketing
		...
		oBrush := oBruCO
		oPrn:FillRect({ nVIL+1379 , 100 , nVIL+1459 , 2150 } , oBrush , '01' )
	
		oFont :=  TFont():New( 'Courier new' , , -17);	oFont:Bold 	:= .T.
		oPrn:Say( 		nVIL+1434 , 110 , cMsgMark , oFont ,   , 0 , )
...
//| ***************************  Textos Titulos Outros  ********************************* 
	...
	oFont :=  TFont():New( 'Courier new' , , -12);	oFont:Bold 	:= .T.
	oPrn:Say( nVIL+2369 , 630 , 'Nº de Peças:' , oFont ,   , 0 , )
...
*******************************************************************************
	...
	Local cPara  := ""
	Local cCopia := ""
	Local cPathL  := Alltrim( oPrn:cPathpdf )
	Local cFile  := Substr(Alltrim(oPrn:cFileName),1,Len(Alltrim(oPrn:cFileName))-4)
	Local cExt   := ".pdf"
	Local bValSent := {|| U_VSend060(cFile+cExt) }
	Local cPathS	:= "\system\anexos\"
	...
	cAssunto	:= Iif(lEhOrc , 'Orcamento de Venda Imdepa' , 'Pedido de Venda Imdepa' )
	cAnexo 		:= cPathS + cFile + cExt
	
	StartJob("U_CopyF060",GetEnvServer() ,.F. ,"Data Atual " + cValToChar( Date() ) ,cPathL+cFile+cExt )
	
		//|         _cFrom,_cTo  ,_cBcc  ,_cSubject,_cBody,_cAttach,lTela
	U_EnvMyMail( cDe , Lower(cPara) , lower(cCopia) , cAssunto , 'Segue anexo '+ cAssunto , cAnexo , .T., .F., bValSent)

Return()
*******************************************************************************
User Function VSend060(cFile)
*******************************************************************************
Local cPathServer := "\system\anexos\"
Local lVerifica := .F.

lVerifica := File(cPathServer + cFile)

If !lVerifica
	Iw_MsgBox("Aguarde uns segundos... O arquivo "+cFile+" esta sendo Anexado ao e-mail...","Atencao","ALERT")
EndIf

Return(lVerifica)
*******************************************************************************
User Function CopyF060(cAttach)
*******************************************************************************
	Local cDrive			:=  ""
	Local cDiretorio		:=  ""
	Local cNome				:=  ""
	Local cExtensao			:=  ""
	Local cPathInServer	 	:= "\system\anexos\"
// Divide ecaminho e nome do arquivo....anexo
