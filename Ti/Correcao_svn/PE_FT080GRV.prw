...
For nX := 1 To Len(aCols) 
	...
	If aCols[nX][ Len(aCols[nX]) ] .OR. aCols[nX][nPACP_REC] > TMPACP->RECLAST .OR. aCols[nX][nPACP_REC] == 0    
	...
	For nY := 1 To Len(aDadACP[2])
		If ValType(aDadACP[2, nY]) <> "L"
			...
			If aCols[nX][ Len(aCols[nX]) ] // Verifica se esta deletado o Registro.
				ZLP->ZLP_STYLE := "E"
			ElseIf aCols[nX][nPACP_REC] > TMPACP->RECLAST .Or. aCols[nX][nPACP_REC] == 0 
				ZLP->ZLP_STYLE := "I"
			Else
				ZLP->ZLP_STYLE :="A"
			EndIf	
