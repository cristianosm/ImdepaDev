...
±±ºDesc.     ³ Rotina xpara Importar, Ajustar Planilha de Curva.           º±±
±±º          ³                                                            º±±
±±º          ³                                                            º±±
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	...
	DbSeek('IM1_GRPMAR'	); 	Aadd(aHeader,{ Trim(X3_TITULO),  X3_CAMPO	, X3_PICTURE, X3_TAMANHO, X3_DECIMAL, '', X3_USADO, X3_TIPO, X3_F3  })
...
/*Do While !Eof() .And. (X3_ARQUIVO == cTabela)
	If X3USO(X3_USADO) .And. cNivel >= X3_NIVEL
		If AllTrim(SX3->X3_CAMPO) == 'IM1_SEGMEN'
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	...
	//MemoWrit(GetTempPath()+"Mod2IM1.TXT", cQuery)
		...
		Do While !Eof()
			...
			Aadd(aCols, {	SQL->IM1_GRPMAR, SQL->IM1_COD, 	SQL->IM1_DESC,;
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	...
	// Chamado: 1099 GLPI - Cristiano Machado - Não existe na tabela  [ZZV] os Segmentos [MNT , REV , OEM]
	cDesSeg   := IIF(cOpcao!= 'INCLUI', Posicione('ZZV',1,xFilial('ZZV')+IM1->IM1_SEGMEN, 'ZZV_DESC'), Space(TamSx3('IM1_DESSEG')[01]))


	If Alltrim(cSegmento) == "MNT"
		 		cDesSeg  := "MATUTENCAO"
	ElseIf Alltrim(cSegmento) == "REV"
		 		cDesSeg  := "REVISAO"
	ElseIf Alltrim(cSegmento) == "OEM"
		 		cDesSeg  := "ORIGINAL"
	Else
		 		cDesSeg  := " "
	EndIf
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	...
	aCordW := { D(125),D(100),D(600),D(735)}
...
*********************************************************************
aRet	  := 	{}
nPGrpMar  :=	Ascan(aHeader,{|x| AllTrim(x[2]) == 'IM1_GRPMAR' 	})
nPCod	  :=	Ascan(aHeader,{|x| AllTrim(x[2]) == 'IM1_COD'		})
		...
		If Ascan(aCReplic, {|X| X[1] != '' } ) > 0
			For nR := 1 To Len(aCReplic)
				If !Empty(aCReplic[nR][nPosMarca])
					...
					DbSelectArea('IM1');DbSetOrder(2);DbGoTop()		//	IM1_FILIAL+IM1_SEGMEN+IM1_COD
					If DbSeek(xFilial('IM1')+cReplicSeg+aCols[nLinha][nPCod], .F.)
						RecLock('IM1', .F.)
							DbDelete()
					    MsUnLock()
                    EndIf
...
Else
	...
	RecLock('IM1', lGrava)
		...
		IM1->IM1_GRPMAR	:=	aCols[nLinha][nPGrpMar]
...
oCBox	   := TComboBox():New( D(039),D(008),{|u| If(PCount()>0,cCBox:=u,cCBox)},aGrpSeg,D(080),D(010),oDlg1,,{|| },,CLR_BLACK,CLR_WHITE,.T.,,"",,,,,,,cCBox )
...
Return()
*********************************************************************
Static Function ImportCurva(cGrpSeg)
*********************************************************************
...
If File(cDiretorio+cArqCsv)
	...
	If MsgYesNo('Confirma a importadação desse '+cArqCsv+' para o '+ENTER+'Grupo de Segmento '+cGrpSeg +' ?')

		cCodSeg		:=	Left(cGrpSeg, TamSx3('ZZV_COD')[01])
		cDescIM1	:=	Substr(cGrpSeg, (TamSx3('ZZV_COD')[01])+4)
		...
		Do While !FT_FEOF()
			...
			If Left(cLinha, 01) != ';'
				...
				cGrupo		:=	Left(cLinha, AT(';',cLinha)-1); cLinha := SubStr(cLinha, AT(';',cLinha)+1)	//	GRUPO DE MARCAS 3
				...
				cIMCMin		:=	Left(cLinha, AT(';',cLinha)-1); cLinha := SubStr(cLinha, AT(';',cLinha)+1)	//	IMC Mínimo
				...
				cMkpPro4	:=	IIF(AT(';',cLinha)==0, cLinha, Left(cLinha, AT(';',cLinha)-1) )				//	MUP MÍN PROMO 4 35d c/UA

				DbSelectArea('IM1');DbSetOrder(2);DbGoTop()
				If DbSeek(xFilial('IM1') + cCodSeg + cCurva, .F.)
					lGrava := .F.
				Else
					lGrava := .T.
				EndIf
...
If cOpcao == 'EXCLUI'	//	PARA EXCLUI_TODOS O USUARIO DEVE IR SEGMENTO A SEGMENTO.
	...
	cQuery += "	FROM	" +RetSqlName('IM1')+"  "+ENTER
	cQuery += "	WHERE	IM1_FILIAL	 = 	'"+xFilial('IM1')+"'	"+ENTER
	cQuery += "	AND		IM1_GRPMAR	 =	'"+IM1->IM1_GRPMAR+"'  	"+ENTER
...
***********************************************************************
	...
	If nPosSeg > 0
		If AllTrim(IM1->IM1_SEGMEN) != AllTrim(oBrwR:aCols[oBrwR:nAT][nPosSeg])
...
Return Int(nTam)
***********************************************************************
Static Function D(nTam)	//	QDO TELA Eh DESENVOLVIDA COM RESOLUCAO 1080 	// aCGD:={D(44),D(05),D(118),D(315)}
***********************************************************************
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³ Ajusta Tamanho dos Objetos            ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
...
User Function GravaLogCurva(nLinha, cOpcao)
	...
	Local aDadTOP  := {{ "IM1_GRPMAR", "IM1_COD", "IM1_DESC", "IM1_MARKUP", "IM1_ALCMAX", "IM1_ALCMED", "IM1_IMCMIN", "IM1_MKPM35", "IM1_MKPPR1", ;
...
EndSql	  */
	...
	If cOpcao == "ALTERA"
		aAdd(aDadTOP[2], Iif(IM1->IM1_GRPMAR	<>	aCols[nLinha][nPGrpMar], IM1->IM1_GRPMAR, .F.	) )
