...
//| Caracteres
	SetPrvt("cTextoAux,cCodProd,cTitle,cMens,cRef,cMar,cCod,cDes,cGrp,cGr1,cGrpM3,cCodIte,cGr2,cGr3,cSeg,cFab,cVei,cMod,cGpa,cNov,cI32,cI33,cI34,cI35,cI51,cI52,cI53,cFilEst")
	Store "" 			To cMens
	Store Space(06) 	To cGrp,cGr1,cGr2,cGr3,cGrpM3,cCodIte
...
//| Dados para a pasta "Dados Cadastrais"
	...
	oCod  := TGet():New( 040,043,{|u| If(PCount()>0,cCod:=u,cCod)	},oFolder:aDialogs[1],040,010,'', {|| cCod := IIF(Empty(cCod),Space(TamSx3('UB_PRODUTO')[01]), PadL(AllTrim(cCod),08,'0')), oCod:Refresh() }, CLR_BLACK,CLR_WHITE,/*oFont1*/,,,.T.,,,,.F.,.F.,,.F.,.F.,"SB1","",,)	// FABIANO PEREIRA - SOLUTIO
...
//|Precos de Campanha
	...
	oSayProc := TSay():New( 030, 010,{|| cMsgAguarde },oDlg02,,/*oFont*/,.F.,.F.,.F.,.T.,CLR_GRAY,CLR_WHITE,080,008 )	//	FABIANO PEREIRA - SOLUTIO
	oSayProc:Hide()
...
If FUNNAME() <> "IMDB2BPV"
	...
	SB1->( DbSeek( xFilial( "SB1" ) + M->UB_PRODUTO ) )
   	 ...
   	 If Alltrim (SB1->B1_MARCA) $ "FAG/TIMKEN/FRM/INA/FCM/BURGER"
...
*******************************************************************************
	...
	cVarTeste 	:= cRef+cMar+cCod+cDes+cGrp+cGr1+cGr2+cGr3+cSeg+cFab+cVei+cMod+cGpa+cNov+cGrpM3+cCodIte // FABIANO PEREIRA - SOLUTIO
...
// ReInicializa as variaveis em razao do Alltrim aplicado anteriormente
	Store Space(06) 	To cGrp,cGr1,cGr2,cGr3,cGrpM3,cCodIte // FABIANO PEREIRA - SOLUTIO
	Store Space(15)	To cCod
	Store Space(20)	To cRef,cMar,cFab,cNov
	Store Space(30)	To cMod,cSeg
	Store Space(40)	To cDes,cVei,cGpa
...
*******************************************************************************
	...
	IIf(!Empty(cGr3),cQuery +=				"AND SB1.B1_SGRB3 	=     	'" 	+cGr3+ "' ",)		//	 FABIANO PEREIRA - SOLUTIO
...
Return()
******************************************************
Static Function FEstCorreias(cCodCorreia)
******************************************************
	Local cQtdRet 	:= ''
	Local nQtdRet	:=	0
	Local _cBase  	:= A093VldBase(cCodCorreia)



//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³  VERIFICA TAMANHO DO CODIGO PARA PESQUISA	³
//³  FAMILIA + MATERIAL + REFERENCIA         	³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	_nPosFim := 2
	_aIdSBQ	 := A093ORetSBQ(_cBase)
	For nFor :=1 To Len(_aIdSBQ)
		If AllTrim(_aIdSBQ[nFor][1]) == "LRG"
			Exit
		EndIf
		_nPosFim +=_aIdSBQ[nFor][2]
	Next



	IIF(Select('TMPEST')!=0, TMPEST->(DbCloseArea()), )
	cSql := " SELECT SB2.B2_COD, "+ENTER
	cSql += " 		(SB2.B2_QATU - SB2.B2_RESERVA - SB2.B2_QACLASS- SB2.B2_QPEDVEN) QTDEST  "+ENTER+ENTER

	cSql += " FROM 	"+RetSqlName("SB2")+" SB2 				"+ENTER
	cSql += " WHERE 	SB2.B2_FILIAL = '"+xFilial('SB2')+"' 	"+ENTER
//cSql += " AND 	SB2.B2_LOCAL IN('01','02')				"+ENTER
	cSql += " AND 	SB2.B2_LOCAL IN('02')				"+ENTER
	cSql += " AND 	SUBSTR(SB2.B2_COD,1,"+AllTrim(Str(_nPosFim))+") = '"+Left(cCodCorreia,_nPosFim)+"'	"+ENTER
	cSql += " AND 	(SB2.B2_QATU - SB2.B2_RESERVA - SB2.B2_QACLASS- SB2.B2_QPEDVEN) > 0 				"+ENTER
	cSql += " AND 	SB2.D_E_L_E_T_ != '*'	"+ENTER
	cSql += " ORDER BY B2_COD 				"+ENTER

	DbUseArea(.T.,"TOPCONN",TcGenQry(,,cSql),'TMPEST',.T.,.T.)



//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³  VERIFICA LARGURA DO PRODUTO ATUAL	³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	aVarSBS  := A093SBSVars(cCodCorreia)
	...
	//³  VERIFICA LARG.ATUAL X LARG.SB2 (MENOR -> MAIOR) ³
	//|  01106006050    <- PROD.ATUAL (M->UB_PRODUTO)	 |
	//|  01106006075    								 |
	//|  01106006100    								 |
	//|  01106006730    								 |
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		DbSelectArea('TMPEST');DbGoTop()
		Do While !Eof()
			If AllTrim(TMPEST->B2_COD) == AllTrim(cCodCorreia)
				nQtdRet += TMPEST->QTDEST
			Else

				aVarSBS := A093SBSVars(TMPEST->B2_COD)
				nPos 	:= Ascan(aVarSBS, {|X|  AllTrim(X[01]) == 'LRG'} )
				If nPos > 0

					nLargSB2 := Val(aVarSBS[nPos][03])
					If Mod(nLargSB2, nLargAtu) == 0
						nQtdRet  += (nLargSB2 / nLargAtu)    * TMPEST->QTDEST 	//	EX.: 100 / 0.50 = 2
					Else
						nQtdRet  += Int(nLargSB2 / nLargAtu) * TMPEST->QTDEST	//	APENAS O INTEIRO DO RESULTADO "QUEBRADO" EX.: (0.75/.050 = 1.5)
					EndIf
