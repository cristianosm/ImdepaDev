...
If DbSeek(xFilial('SUA') + SC5->C5_NUMSUA, .F.)
	...
	lIM1TravaM	:=	IM1->(FieldPos('IM1_TRAVAM')) > 0
	lNewIMC		:=	IM1->(FieldPos('IM1_NEWIMC')) > 0
	lNewCpo		:=	.F.
    
    If lNewIMC .And. lIM1TravaM
    	nPos 	:=	Ascan(aAmbiente, {|X| AllTrim(X[01]) == AllTrim(Upper(GetEnvServer())) })
    	lNewCpo	:= 	IIF(nPos > 0, cFilAnt $ aAmbiente[nPos][02] .Or. '*' $ aAmbiente[nPos][02], .F.)
    EndIf
	...
	If lNewCpo
		...
		//|	IM1_IMCMIN COM NOVA FORMATACAO 		|
		//| EX.: 0,12 = 12%						|
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		cQuery += "		(IM1.IM1_NEWIMC * 100) MKP_IMCMIN,										"+ENTER
		cQuery += "		(IM1.IM1_TRAVAM * 100) TRAVAM,											"+ENTER

	Else
		//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
		//|	IM1_IMCMIN COM FORMATACAO ORIGINAL	|
		//| EX.: 1,12 = 12% = 0,12				|
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ	
		cQuery += "			((IM1.IM1_IMCMIN -1)*100) MKP_IMCMIN,								"+ENTER
	EndIf

	cQuery += "			SUB.UB_TABPRC															"+ENTER+ENTER
	...
	Do While !Eof()
		...
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ	
		nMvImcMin := IIF(lNewIMC, TMPL->TRAVAM, nMvImcMin)
		...
		If lDescAcima
			...
			ElseIf nImcRProd <  nMrkImCMin .And. nImcRPedido >= nImcRMinRGV
				Aadd(aGerente, {cItemAT, cProdAT, cDescProd, 'DESCONTO > ALÇADA e IMCR PRODUTO < IMC.MIN e IMCR PEDIDO > MIN.PERMITIDO', 'LIB.GER', nImCProd, nImcRProd, cItTabPrc, .F.})	//MRK.MIN
				cLiber	:=	'LIB.GER'
			Else
				Aadd(aGerente, {cItemAT, cProdAT, cDescProd, 'DESCONTO: '+cValToChar(nDescVenda)+'%   ACIMA DO  PERMITIDO: '+cValToChar(nDescMax)+'% ', 'LIB.DIR'/*'BLQ.GER'*/, nImCProd, nImcRProd, cItTabPrc, .F.})
				cLiber	:=	'LIB.DIR'
			EndIf
			...
			If nImcRPedido < nImcRMinRGV
				...
				If lNextLevel
					...
					Aadd(aDiretor, {cItemAT, cProdAT, cDescProd,;
								  	...
								  	'LIB.DIR', nImCProd, nImcRProd, cItTabPrc, .F.})
...
Define Dialog oDlgBlq Title "Detlhes Bloqueio - Pedido: "+SC5->C5_NUM+"  |  Dt.Emissão: "+DtoC(SC5->C5_EMISSAO) From 120,120 To 415,1400 Pixel 
	...
	cVinculo	:=	AllTrim(SA1->A1_VINCULO)+' - '+AllTrim(Posicione('CC1', 1, xFilial('CC1')+SA1->A1_VINCULO, 'CC1_DESCRI'))
