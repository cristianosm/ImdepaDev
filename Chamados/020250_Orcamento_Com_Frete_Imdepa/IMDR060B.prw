...
#Define POS_M_FRET 15	//| Linha entre os itens que sera imprema a mensagem de Frete
...
Static Function ConfigVar(_lPerg) //| Configura as Variaves necessarias
	*******************************************************************************
    ...
    _SetOwnerPrvt( 'cFrCIB'	, 'R$ 25,17' /*CFRTCIFA1*/ ) //| Frete CIF Imdepa Mais Barato
    _SetOwnerPrvt( 'cFrCIR'	, 'R$ 32,45' /*CFRTCIFB1*/ ) //| Frete CIF Imdepa mais Rapido 

    _SetOwnerPrvt( 'cFCIBPE'	, '3' /*CFRTCIFA2*/ ) //| Frete CIF Imdepa mais Barato Prazo de Entrega
    _SetOwnerPrvt( 'cFCIRPE'	, '1' /*CFRTCIFB2*/ ) //| Frete CIF Imdepa mais Rapido Prazo de Entrega
...
Static Function DMarketing() //| Verifica e Monta as Mensagens de Marketing
	*******************************************************************************
	...
	If !( M->UA_TPFRETE=='C' ) //| Em caso de Frete FOB deve Imprimir mensagem Frete CIF
		lMsgFrete := .T.
	EndIf 
...
Static Function PItens(oPrn, aItem)
	*******************************************************************************
	...
	If lMsgFrete .And. nI >= POS_M_FRET .And. EvAL(bPriPag)
		nPIMM := 7
	ElseIf !EvAL(bPriPag) /// Apenas na Primeira Pagina deve ser impresso 
		lMsgFrete := .F. 
	EndIf
	...
	//| Chamado : 18681 -> Ajuste no orçamento e tela da verdade - Gabriel
	//| Correias - Referencia Gates
	//oFont := TFont():New( 'Courier new' , , -7);	oFont:Bold := .T.
	//oPrn:Say( eVal(bPoS)-2 , 347+nVIC  ,  PADL(aItem[I_REFGATES],35) , oFont , , 0 , )
...
Static Function PMarketing(oPrn)//| Imprime a Mensagem de Marketing ...
	*******************************************************************************
	...
	If lMsgFrete  //| Verifica se deve imprimir a Mensagem de Frete CIF Imdepa

		
		// cFrCIB //| Frete CIF Imdepa Mais Barato
		// cFrCIR //| Frete CIF Imdepa mais Rapido 

		// cFCIBPE //| Frete CIF Imdepa mais Barato Prazo de Entrega
		// cFCIRPE //| Frete CIF Imdepa mais Rapido Prazo de Entrega
		...
		oFont :=  TFont():New( 'Courier new' , , -21) 
		oFont:Bold 	:= .T. 
		oPrn:Say( 1960 , 470-nAC , '*** Aproveite o Frete pela Tabela Imdepa *** ' , oFont ,   , 255 , ) 
		oFont :=  TFont():New( 'Courier new' , , -16) 
		oFont:Bold 	:= .T. 
		oPrn:Say( 2030 , 500-nAC , '1. Opcao Melhor Preço '+cFrCIB+' e Prazo de Enrega '+cFCIBPE + Iif( Val(cFCIBPE) > 1, ' Dias ', ' Dia ' ), oFont ,   , 255 , ) 
		oFont :=  TFont():New( 'Courier new' , , -16) 
		oFont:Bold 	:= .T. 
		oPrn:Say( 2080 , 500-nAC , '2. Opção Melhor Prazo de Entrega ' + cFCIRPE + Iif( Val(cFCIBPE) > 1, ' Dias ', ' Dia ' ) + 'e Preço '+cFrCIR , oFont ,   , 255 , ) 
		oFont :=  TFont():New( 'Courier new' , , -20) 
		oFont:Bold 	:= .T. 
		oPrn:Say( 2150 , 750-nAC , '!!! Consulte seu Vendedor !!!' , oFont ,   , 255 , ) 
		oFont :=  TFont():New( 'Courier new' , , -14) 
		oFont:Italic 	:= .T. 
		oPrn:Say( 2198 , 550-nAC , '\\\ Atençào : Frete sem Impostos - Prazo de Entrega Previsto /// ' , oFont ,   , 255 , )
