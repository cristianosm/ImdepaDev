...
±±ºPrograma  ³PE_TK271BOK?Autor ³Edivaldo            ?Data ? 30/08/13   º±?
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Ponto de entrada executado na confirmação do ATENDIMENTO.    º±?
±±?         ?                                                            º±?
±±?         ³Esse ponto de entrada ?chamado no botão "OK" da barra de    º±?
±±?         ³ferramentas da tela de atendimento do Call Center, antes da  º±?
±±?         ³função de gravação.                                          º±?
...
*******************************************************************************
	...
	Return lReturn
*******************************************************************************
Static Function TeleVendas(lReturn) // Tarefas Relacionadas ao Televendas
*******************************************************************************
	
	/* Desabilitado Log com o tempo de Processamento na Digitação dos Itens no TeleVendas
	If nQtdLogDig>0
	If Len(Acols)>=nQtdLogDig
	U_IMDA2000(aLogItens,nTimeBotao)
	aLogItens:={}
	nTimeBotao:=0
	EndIf
	EndIf
	*/
	...
	//Edivaldo Gonçalves (Tratamento para Atendimento já gravado e com Nota Fiscal Emitida)
	If !__COPIATT
		If (!Empty(M->UA_NUMSC5))

			//Checa se o Pedido possui Nota Fiscal Emitida
			If SUA->(CheckPedNota(M->UA_NUMSC5))
				__Podesair:= "S" //Libera a saída da rotina em caso do operador ter redigitado a quantidade no atendimento com NF Emitida
				MessageBox("Este Atendimento possui Nota Fiscal Emitida  !"+CHR(10)+CHR(13)+"Utilize o botão <Fechar> ou <Limpa o Atendimento>","Atendimento com Nota Fiscal",MB_ICONEXCLAMATION)
				Return (.F.)
			Endif
		Endif
	Endif
	...
	*/
	...
	If lAbaTeleVendas
		...
		oGetTlv:oBrowse:setfocus()  //Seta o Foco no Objeto Acols
		...
		If (M->UA_OPER=='1' .AND. M->UA_TPFRETE=='C' .AND. !_ljaCPaCP )
			IW_MsgBox("Para continuar , clique na Condição de pagamento ! ","Gestão de Fretes", "ALERT")
			aValores[4]  := 0
			M->UA_FRETRAN :=0
		...
		Aviso("Valor do Frete","Não ?permitido a alteração do valor do frete !",{"OK"} )
		...
		****Clientes estratégicos identificados e justificados pela diretoria comercial.
		Esta justificativa dever?ser encaminhada para análise
		...
		*/
		...
		IF !LVINCULO  .AND. SA1->A1_CLIFRTE<>'1'
			...
			IF (SA1->A1_TPFRET <> 'F') .AND. !LCLIENTESP .AND. !LCLISPFRT
				...
				IF ((AVALORES[4]< M->UA_FRETCAL) .AND. NDESTACA=1)
					...
					IW_MsgBox("Frete "+CTEXTO+" nao pode ser menor que Calculado ! ","Gestão de Fretes:Nao sao CM/PP ou Especial", "ALERT")
				ENDIF
			ENDIF
			...
			IF (SA1->A1_TPFRET <> 'F') .AND. LCLIENTESP .AND. LCLISPFRT
				...
				IF ((AVALORES[4]< M->UA_FRETCAL) .AND. NDESTACA=1)
					...
					IW_MsgBox("Frete "+CTEXTO+" nao pode ser menor que Calculado ! ","Gestão de Fretes:Nao sao CM/PP ou Especial CFSP", "ALERT")
				ENDIF
			ENDIF
		ENDIF
		...
		IW_MsgBox("Frete "+CTEXTO+" nao pode ser menor que Calculado ! ","Gestão de Fretes: CM/PP ou Especial", "ALERT")
		ENDIF
		ENDIF
		*/
		...
		IF SA1->A1_TPFRET = 'F'
			IF ((AVALORES[4]< M->UA_FRETCAL) .AND. NDESTACA=1)
				...
				IW_MsgBox("Frete "+CTEXTO+" nao pode ser menor que Calculado ! ","Gestão de Fretes: Regra Cliente FOB", "ALERT")
			ENDIF
		...
		Else //Se frete nova versao 2.0
			...
			*/
			...
			if (aValores[4] < _NVALSUG) .AND. M->UA_TPCIF='CIFD'   ///CIF Destacado
				IF _NACOL>1
					ALERT( "Voc?selecionou uma transportadora cujo frete não ?o menor entre as transportadoras."+CHR(13)+CHR(13)+"Não ?possível diminuir o valor do frete neste caso.")
				ELSE
...
*******************************************************************************
	...
	Local cCond
	Local lOk := .T.
	Local cRisco := U_ConvA1toUS("SA1->A1_RISCO")

	If cRisco $ GETMV("MV_RISCOBL")
		...
		If nDias > GetMv("MV_DIASULP")
			...
			MsgAlert("Cliente risco " + cRisco + " com condição de pagamento com data da última parcela maior que " + ALLTRIM(STR(GetMv("MV_DIASULP"))) +" dias! Bloqueio Financeiro!","Alerta")
...
Return (lOk)
*******************************************************************************
Static Function CheckPedNota(cNumPedC5)
*******************************************************************************
	Local lTemNf:=.F.
	Local aArea := GetArea()
	...
	DbSelectArea("SC5")
	DbSetOrder(1)
	If DbSeek(xFilial("SC5")+cNumPedC5)
