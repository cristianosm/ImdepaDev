...
User Function IMDR060B(_lPerg)
	*******************************************************************************
	...
	bValida := {|| lContinua, lContinua := Valida() } //| Bloco com Valida oes Necessarias...
...
Static Function Valida()//| Valida es Necessarias
	*******************************************************************************
	...
	If !ExistDir("C:\MP11")
		If MakeDir ("C:\MP11" ) <> 0
			IW_MsgBox("Nao foi possivel criar o Diretorio 'C:\MP11' este necessario, para salvar o arquivo PDF. Entrar em contato com a TI.","Impressao de Orcamento/Pedido", "ALERT")
			lValidacao := _SAIR_
		EndIf
	EndIf
	...
	If lValidacao .And. _lAltOrc .And. !lPerg //.And. Alltrim(Procname(1)) <> "U_TMKVFIM"//.And. Empty(M->UA_DOC) .AND. PROCNAME(2) == "ACTIVATE" //PROCNAME(1)<> "U_TMKR03" //PROCNAME(1) <> "U_TMKR3A"
		IW_MsgBox("Como este Or amento/Pedido foi alterado, realize a gravacao antes da Impressao","Impressao de Orcamento/Pedido", "ALERT")
		lValidacao := _SAIR_
	Endif

	IF !_ljaCPaCP .And. lValidacao
		IW_MsgBox("Por favor, clique na Condicao de pagamento antes de imprimir o Orcamento","Impressao de Orcamento/Pedido", "ALERT")
		lValidacao := _SAIR_
	EndIf
...
Static Function DMsgens(aMens)//| Monta as Mensagens de Observacoes e Rodape
	*******************************************************************************
	...
	Local MsgRodape  := SuperGetMV("IM_MSROORC", .F., '[Emitido em '+ Dtoc(dEmisOrc) +' - Proposta Valida por 2 Dias - Sera Acrescida Despesa Bancaria por Duplicata ] - Visite nosso site WWW.IMDEPA.COM.BR', Nil )
	...
	//| Mensagem Linha 01
	aMens[M_LINHA1] := '* Faturamento Minimo de R$ ' + Alltrim(Transform( GetMv("IM_PEDMINF"),'@E 999,999.99')) +' Reais'

	//| Mensagem Linha 02
	aMens[M_LINHA2] := '* Para compras fora do estado e fundamental levar em consideracao as mesmas bases de ICMS e ST.'
	...
	*******************************************************************************
	...
	If !lMsgMark
		If ( ( SB1->B1_SGRB1 == "220000" .And. SB1->B1_GRMAR3 == "000510" .And. SB1->B1_MERCADO == "N" ) .Or. ( !SUA->UA_PROSPEC .And. SA1->A1_GRPSEG == "AG3" ) )
			lMsgMark := .T.
			cMsgMark := 'Correias Agricolas CONTINENTAL: unicas com 100% de garantia do fabricante !!!'
...
Static Function PMsgens(oPrn)//| Imprime Mensagens
	*******************************************************************************
	...
	oPrn:Say( nVIL+2029 , 2000 , "Pagina: " + cValToChar(nPagina) + "-" + cValToChar(nTotPag) , oFont , , 0 , )
...
Static Function PLayout(oPrn) //|Imprime Layout Completo do Or amento
	...
	*/
	...
	//| *************************** Textos Titulo *********************************
	oFont := TFont():New( 'Courier new' , , -20);	oFont:Bold := .T.
	oPrn:Say( 115 , 70 , Iif( lEhOrc , 'ORCAMENTO', ' PEDIDO ' ) , oFont , , 255 , )
	...
	If eVal(bPriPag)
		...
		oFont := TFont():New( 'Courier new' , , -10);	oFont:Bold := .T.
		oPrn:Say( nVIL+570 , 50 , 'ENDERECO: ' , oFont , , 0 , )
		...
		oFont := TFont():New( 'Courier new' , , -10);	oFont:Bold := .T.
		oPrn:Say( nVIL+570 , 1180+(nVIC/2) , 'ENDERECO: ' , oFont , , 0 , )

	EndIf

	//| *************************** Textos Titulo Itens *********************************
	oFont := TFont():New( 'Courier new' , , -12);	oFont:Bold := .T.
	oPrn:Say( nFPG+nVIL+750 , -5+nVIC , 'Codigo' , oFont , , 0 , )

	oFont := TFont():New( 'Courier new' , , -10);	oFont:Italic := .T.
	oPrn:Say( nFPG+nVIL+770 , 10+nVIC , 'Item' , oFont , , 0 , )

	oFont := TFont():New( 'Courier new' , , -12);	oFont:Bold := .T.
	oPrn:Say( nFPG+nVIL+750 , 325+nVIC , 'Descricao' , oFont , , 0 , )
	...
	oFont := TFont():New( 'Courier new' , , -12);	oFont:Bold := .T.
	oPrn:Say( nFPG+nVIL+770 , 970+nVIC , 'Unitario' , oFont , , 0 , )
	...
	oFont := TFont():New( 'Courier new' , , -12);	oFont:Bold := .T.
	oPrn:Say( nFPG+nVIL+770 , 1470+nVIC , 'Unitario' , oFont , , 0 , )

	oFont := TFont():New( 'Courier new' , , -12);	oFont:Bold := .T.
	oPrn:Say( nFPG+nVIL+730 , 1642+nVIC , 'Vlr+IPI+ST' , oFont , , 0 , )

	oFont := TFont():New( 'Courier new' , , -12);	oFont:Bold := .T.
	oPrn:Say( nFPG+nVIL+770 , 1662+nVIC , 'Unitario' , oFont , , 0 , )
	...
	oFont := TFont():New( 'Courier new' , , -12);	oFont:Bold := .T.
	oPrn:Say( nFPG+nVIL+730 , 2105+nVIC , 'Previsao' , oFont , , 0 , )
	...
	oFont := TFont():New( 'Courier new' , , -12);	oFont:Bold := .T.
	oPrn:Say( nVIL+2369 , 630 , 'N de Pecas:' , oFont , , 0 , )

	oFont := TFont():New( 'Courier new' , , -12);	oFont:Bold := .T.
	oPrn:Say( nVIL+2369 , 1190 , 'Peso Liquido:' , oFont , , 0 , )
	...
	oPrn:Say( nVIL+2442 , 50 , 'OBSERVACOES:' , oFont , , 0 , )
	//EndIf
