...
#DEFINE POS_TEM_SALDO	1   
#DEFINE POS_TEM_LOCAL	2
#DEFINE POS_CODITE  	3
...
//| Inicializacoes
	...
	cTitle		:= OemToAnsi("Consulta Avan�ada de Produtos")
	cCodProd 	:= &(Readvar())
...
//oTBitmap := TBitmap():Create(oDlg01,30,146,260,184,,'xclose.png',.T.,            {||Alert("Clique em TBitmap1")},,.F.,.F.,,,.F.,,.T.,,.F.)
	...
	oFolder := TFolder():New(1,1,{"Dados Cadastrais ","Aplica��o "},{"oFl1","oFl2",},oDlg01,,,,.T.,.F.,490,095)
...
//| Desenha o Cabecalho das Filiais
	...
	@ nXLinha,004  SAY " "+PadR("DISP.",09)+"  "+PadR("REFERENCIA",20)+"  "+PadR("MARCA",20)+"  "+PadR("R$ Refer�ncia",13)+"  "+PadR("CODIGO",14)+'E'+' '+'S'+ ' ' + c3Cabecalho +'A' PIXEL FONT oFONT
	nXLinha += 102
...
//		ON DBLCLICK (Marca(oListBox:nAt,@aListBox),oListBox:nColPos := 1,oListBox:Refresh()) NOSCROLL PIXEL
    ...
    */
	...
	oBtn_avancado := TButton():New(134,453,"",oDlg01,{|| (IniTela(), oDlg01:End()) },39,18,,,.F.,.T.,.F.,'Avan�ado',.F.,,,.F. )
	...
	oTMsgItem1 	:= TMsgItem():New( oTMsgBar,'F11 Consulta Avan�ada', 204,,,, .T., {||})
...
*******************************************************************************
	...
	cMsg += " " + PADR( "Filial" , 15 ) + _TAB_ + _TAB_ + PADL( "Saldo Disp." , 15 ) + PADL( "  Saldo Transfer�ncia" , 15 ) + ENTER
...
*******************************************************************************
	...
	//Item j� foi selecionado e retirado da Lista
	...
	M->UB_PRODUTO 	:= 	Alltrim(StrTran(SubStr(aArray[nIt],70,13), '|','')) //cristiano alterado -> Alltrim(StrTran(SubStr(aArray[nIt],70,TamSx3('UB_PRODUTO')[01]), '|',''))
...
//|Precos de Campanha
	...
	EndIf
	    ...
	    SB1->(U_F190ItAuto(SB1->B1_COD,substr(SA1->A1_GRPSEG,3,1)))
		

	If nQtdAux > 0
		aArray[nIt] := Substr(aArray[nIt],1,7)+' �'+Substr(aArray[nIt],10,200) // faz isso na valida��o
	Endif
...
*******************************************************************************
	...
	Local cPrevTes        	:= ' ' // TES autom�tica prevista para o produto em quest�o(ver FPARTES())
	Local nAcreMva        	:= 0   // Acr�scimo do MVA
	Local nAcreAuxMva       := 0   // Acr�scimo Auxiliar MVA
	Local lTemAcreMva     	:= .F. // Vari�vel para definir se a filial possuiu ou n�o o Acr�scimo MVA
	Local nDesAcreFil     	:= 0   // Desconto/Acr�scimo por filial conforme tabela ZA7 (ZA7_PROMES)
...
// ReInicializa as variaveis em razao do Alltrim aplicado anteriormente
   ...
   //Verifica se a filial corrente possui o Acr�scimo MVA ou Acre/Desc especifico
	/*
	FABIANO PEREIRA - SOLUTIO
...
//| Abri e Ordena Tabelas
	...
	*/
	...
	Else
		...
		EndIf
		*/
		//Inserido por Edivaldo Gon�alves em 22/07/2015
	If ! lProspect	
		cGrpSeg	:= 	SA1->A1_GRPSEG
    Else 
    ...
    Endif
		...
		If nPosTab > 0 
			...
			If M->UA_TABELA !=	aTabela[nPosTab][02]
				...
				nPrecoPro   := 	ExecBlock("OM010PRC",.F.,.F.,{cTabPreco,cQuery->B1_COD,1,M->UA_CLIENTE,M->UA_LOJA,M->UA_MOEDA,dDataBase,1})  //	Tipo (1=Pre�o/2 = Fator de acr�scimo ou desconto)
	...
	//| Solicitatdo pelo otoni para n�o mostras somente os bloqueados no cad prod
	...
	//|Ordena o array AlistPRO por ordem de nValOrdem
		aSort(aListPRO,,,{|a,b| a[2]+a[1] > b[2]+b[1] })  // nValOrdem + string  --> usar esta linha somente se tiver usando a fun��o Queryimdf190()
		aListClone  := {}
	...
	If nCount > 0
		cMens := alltrim(Str(nCount)) +" Registros n�o mostrados por n�o possuirem estoque, nem produto Equivalente nem Similar"
	Else
		cMens := ""
	Endif
...
//Dados para a pasta "Dimensionamento6"
	...
	@ 110,004  SAY PadR("QTD",09)+"  "+PadR("REFERENCIA",20)+"  "+PadR("MARCA",20)+"  "+PadR("R$ Refer�ncia",13)+"  "+PadR("CODIGO",15)+' '+;
...
*******************************************************************************
	...
	If nPos > 0 
		...
		If nPrecoTab == 0 //.And. Empty(A093VldBase(cProduto))
			MsgAlert("Produto "+AllTrim(cProduto)+" pre�o ZERADO."+ENTER+"Entre em contato com Depto SUPRIMENTO.")
...
//Endif
	...
	//	IW_MsgBox(Oemtoansi("Produto "+M->UB_PRODUTO+" n�o cadastrado ou sem pre�o para esta tabela de pre�os."),'Aten��o','ALERT') // FABIANO PEREIRA - SOLUTIO
	...
	If lOk
		...
		_lAltOrc 	:= .T. 	//| ALTERA��O DE QUANTIDADE ...
...
*******************************************************************************
	...
	//| Cristiano Machado |
	//|Vamos utilizar o AddLinha do IMDF051... QUalquer Melhoria/Correcao deve ser feita l�...|
	lCriou :=  StaticCall( IMDF051 , FAddLinha )
	If lCriou
			If M->UA_TPFRETE=='C'
			  //lSelRotas:=.T.  //Criou uma nova linha , obriga o clik na condi��o de pagamento para o calculo correto do Frete
			  _ljaCPaCP     :=.F. 
			   aValores[4]  := 0
			  M->UA_FRETRAN :=0    
...
//InKeyIdle() <> VK_F11
	...
	IF LEN(aopcoes) == 0
		Alert("Produto " + ALLTRIM(SubStr(aListPRO[oListPRO:nAt],70,14)) + " n�o possui produtos Equivalentes ou Similares")
	Else

		@ 010,001 TO 100,200 DIALOG oDlgX TITLE  OemToAnsi("Selecione a Op��o ")
...
*******************************************************************************
	...
	IF MsgYesNo("Confirma exclus�o deste item ?")
		aListClone	:= {}
...
*******************************************************************************
   ...
   //Local cMarca   := ""
	...
	//Exce��es de Produto X Cliente para n�o gerar a Oferta Autom�tica
...
******
*******************************************************************************
User Function F190ItAuto(cProd,cSegmSA1) //|Itens Automaticos no Orcamento
*******************************************************************************
	Local cQuery   := ""
	Local aArea	   := GetArea()
	Local cMarca   := ""
	...
	If ZAA->( DbSeek( xFilial( "ZAA" ) + cProd ) ) 
	  ...
	  If !Empty(ZAA->ZAA_GRPSG1)
	     If ! (cSegmSA1 $ Alltrim(ZAA->ZAA_GRPSG1) .OR. cSegmSA1 $ Alltrim(ZAA->ZAA_GRPSG2))	       
	        lExec:=.F.
	     Endif 	  	  
	  Endif
	  	  
	  
	  If (ZAA->ZAA_STATUS=='1' .AND. lExec )  //Oferta Habilitada e Valida
	   
	        DbSeek(xFilial("ZAB")+ZAA->ZAA_IDOFER,.F.)		    
		    Do While ZAB->ZAB_FILIAL+ZAB->ZAB_IDOFER == xFilial("ZAB")+ZAA->ZAA_IDOFER .AND. ZAB->ZAB_STATUS=='1' .and. !Eof()
		    
               //Verifica se o Item jao n�o foi incluso na Previsao de Itens para Adicionar ao Or�amento/Pedido 		    
		       If aScan( aCols, {|x| x[nP_PROD] == ZAB->ZAB_SIMILA } ) > 0 	          
	              ZAB->(DBSKIP())
			     Loop
		       Endif

		    		    		    
		    SB1-> ( DbSeek(xFilial( "SB1" ) + ZAB->ZAB_SIMILA,.F.))
		    
		    If nTotIt>1 //Verifica se o Item jao n�o foi incluso na Previsao de Itens para Adicionar ao Or�amento/Pedido 		    
		    		    
		      If ( nPos := aScan( aPreProd, {|x| x[3] == SB1->B1_CODITE } ) ) > 0   //Se j� foi pre-adicionado , avalio o proximo produto		       
		    ...
		    aTemEst:=ZAB->(sElEstItAuto(ZAB->ZAB_SIMILA))
		    
		    If aTemEst[1,POS_TEM_SALDO]
		     AADD(aPreProd,{ZAB->ZAB_SIMILA,aTemEst[1,POS_TEM_LOCAL],SB1->B1_CODITE})
		    EndIf
...
// Para cada registro que localizar, irah colocar na tela !
	...
	// Executa a trigger para preencher os outros campos da tela
   	...
   	// Alimenta a o Local do produto onde h� Saldo
...
******************************************************
Static Function sElEstItAuto(_cCodPro) 
******************************************************
Local _cLoc1   :='01'
Local _cLoc2   :='02' 
Local nSldDisp :=0
...
dbSetOrder(1) // B2_FILIAL+B2_COD+B2_LOCAL
    ...
    //Priorizo o Estoque 01 , se n�o tiver Saldo Consulto 0 02
	If SB2->( MsSeek( xFilial('SB2')+_cCodPro+_cLoc1,.F. ))  
	  nSldDisp := SB2->B2_QATU - SB2->B2_RESERVA - SB2->B2_QACLASS - SB2->B2_QPEDVEN + u_QtdPedTra( SB2->B2_COD, SB2->B2_FILIAL ) 	
	
	      If nSldDisp>0 
	 	    AADD(asElEst,{.T.,_cLoc1,SB2->B2_CODITE}) 
	        Return(asElEst) 
	      EndIf  
	...
	If Len(asElEst)==0  //Procuro Saldo No Armzem  02
	
	   If SB2->( MsSeek( xFilial('SB2')+_cCodPro+_cLoc2,.F. )) 
	         nSldDisp := SB2->B2_QATU - SB2->B2_RESERVA - SB2->B2_QACLASS - SB2->B2_QPEDVEN + u_QtdPedTra( SB2->B2_COD, SB2->B2_FILIAL ) 	 
	
	          If nSldDisp>0 
	 	        AADD(asElEst,{.T.,_cLoc2,SB2->B2_CODITE}) 
	 	         Return(asElEst)	 	  
	          EndIf
	...
	If Len(asElEst)==0 //Retorno Falso se n�o encontrar saldo em nenhum dos Armazens
	  AADD(asElEst,{.F.,' ',' '})  
	Endif  
...
******************************************************
Static Function F190ItInsere(aItensAuto)
******************************************************
	Local aArea	   := GetArea()
	Local npos
	Local nQtdeIt  := Len( aCols )
  ...
  For nItem:=1 To Len(aItensAuto)
	...
	// Alimenta a linha com o Produtok
		__ReadVar     := "M->UB_PRODUTO"
		M->UB_PRODUTO :=aItensAuto[nItem,POS_CODPRO]
	...
	// Executa a trigger para preencher os outros campos da tela
   	...
   	// Alimenta a o Local do produto onde h� Saldo
		__ReadVar 	:= "M->UB_LOCAL"
		M->UB_LOCAL	:= aItensAuto[nItem,POS_TEM_LOCAL]
		aCols[ N, GDFieldPos("UB_LOCAL") ] := M->UB_LOCAL
		GDFieldPut( 'UB_LOCAL', M->UB_LOCAL, N )
