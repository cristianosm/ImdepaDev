...
If !Empty(dDtSchedule)
	...
	*/
	cQuery := "	SELECT  XX1_ROTINA TSK_ROTINA, XX1_CODIGO TSK_CODIGO, XX1_DATA , XX1_HORA TSK_HORA, XX1_STATUS TSK_STATUS "+ENTER	
	cQuery += "	FROM 	XX1								"+ENTER
	cQuery += " WHERE   XX1_ROTINA LIKE '%IMPPRM01%'	"+ENTER
	cQuery += " AND     XX1_STATUS = '0'				"+ENTER
	cQuery += " AND     D_E_L_E_T_ != '*'				"+ENTER
	DbUseArea(.T., "TOPCONN", TCGenQry(,,cQuery), 'TMPTSK', .F., .T.)
	...
	If File(cDirSrv+cArqTabelas) .Or. !lSchedule	// '\csv_import\tab_promo.dtc'
		...
		Do While !FT_FEOF()
			...
			If Left(cLinha, 01) != ';' .And. IsDigit(Left(cLinha, 01))
				...
				//	CODIGO PRODUTO
				cProdCsv	:=	Left(cLinha, AT(';',cLinha)-1)
				cProduto	:=	ChkProduto(cFilCsv, cProdCsv)
 				cProduto	:=	PadR(cProduto, TamSx3('B1_COD')[01])
				cLinha 		:= 	SubStr(cLinha, AT(';',cLinha)+1) 	
...
Else
	...
	If Len(aDA1Erros) > 0
		For nX:=1 To Len(aDA1Erros)
			cErroDA1 += 'FILIAL:  	'+aDA1Erros[nX][01]+Space(05)+;
...
Return()
********************************************************************
Static Function ChkProduto(cFilCsv, cProdCsv)
********************************************************************
Local aArea		:=	GetArea()
Local cProdRet 	:= 	''
Local nTamProd 	:=	GetMv('IM_TAMPROD')
...
//³  VERIFICA SE CODIGO ESTA CORRETO - COM ZEROS PREENCHIDOS	|
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
DbSelectArea('SB1');DbSetOrder(1)   
cPesqProd := PadR(cProdCsv, TamSx3('B1_COD')[01])
If DbSeek(xFilial('SB1') + cPesqProd, .F.)
	cProdRet := AllTrim(SB1->B1_COD)
Else
	//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
	//³  PESQUISA CODIGO COM 8 CARACTERES ³
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	lQuery := .T.
	DbSelectArea('SB1');DbSetOrder(1)
	If Len(Alltrim(cProdCsv)) <= nTamProd
		cPesqProd := PadR(PadL(cProdCsv, nTamProd, '0'), TamSx3('B1_COD')[01])
		If DbSeek(xFilial('SB1') + cPesqProd, .F.)
			cProdRet := AllTrim(SB1->B1_COD)
			lQuery	 := .F.
	    EndIf
	...
	If lQuery
		...
		//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
		//³  PESQUISA VIA QUERY COM LIKE	  ³
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		IIF(Select('TMPSB1')!=0, TMPSB1->(DbCloseArea()), )
		cQuery := "	SELECT  SB1.B1_FILIAL, SB1.B1_COD, SB1.B1_DESC  	"+ENTER	
		cQuery += "	FROM 	"+RetSqlName('SB1')+" SB1					"+ENTER
		cQuery += " WHERE	SB1.B1_FILIAL		=	  '"+cFilCsv+"'	 	"+ENTER
		cQuery += " AND		RTRIM(SB1.B1_COD)	LIKE  '%"+cProdCsv+"'  	"+ENTER
		cQuery += " AND     SB1.D_E_L_E_T_		!= '*'					"+ENTER
		DbUseArea(.T., "TOPCONN", TCGenQry(,,cQuery), 'TMPSB1', .F., .T.)
		DbSelectArea('TMPSB1');DbGoTop()
        Do While !Eof()
			//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
			//³  VERIFICA CARACTER A CARACTER PARA IDENTIFICAR PRODUTO CORRETO	  ³
			//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ

			cProdCsv	:=	AllTrim(cValToChar(Val(cProdCsv)))
			cProdSql 	:= 	AllTrim(cValToChar(Val(TMPSB1->B1_COD)))
			If cProdSql == cProdCsv
				cProdRet := AllTrim(TMPSB1->B1_COD)
				Exit
            EndIf
...
If Empty(cProdRet)
	Aadd(aDA1Erros, {cFilCsv, 'NOT.FOUND', cProdCsv, '', '', 'PRODUTO NAO ENCONTRADO - ChkProduto()'})
EndIf

cProdRet :=	IIF(Empty(cProdRet), cProdCsv, cProdRet)
