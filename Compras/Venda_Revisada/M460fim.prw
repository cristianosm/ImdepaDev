...
/*
Programa M460FIM Autor Raul Data April/03 

Desc. Ponto de entrada apos a gravacao de Nota fiscal 

Alteracao Gravacao dos campos E1_ENDOSSO,E1_CREDICM E E1_REEM, 
campos estes para filtro na geracao do bordero. (O rela- 
cionamento destes campos sao com o arquivo SUA. 
Gravacao do lucro e margem nos arquivos SD2 e SE1 
...
geracao de log. 
MarcioQ.Borg 19/09/07 Ajuste na amarra o do itemSD2 com o acols 
...
//removido em 26/02/2015 em fun o de que as regras de comiss o por operador foram descontinuadas - Rodrigo Carvalho - Solutio
...
/*
COmentado em 26/02/2015 ap s conversar com o Edivaldo - Rodrigo Carvalho Solutio
IF SF2->F2_FILIAL = '13'
U__CORCOMIS() // Corrigi as comiss es de Curitiba com TES com percentual de icms diferido - Agostinho 23/03/2010
ENDIF
*/
...
IF !FOUND() // Compatiblizar para encontrar os titulos gerados com os prefixos das filias, antes da altera o do parametro MV_2DUPREF para SF1->F1_SERIE em 14/02/2013
...
If SF2->F2_CLIENTE==cImdepa .AND. SF4->F4_ESTOQUE = 'S' .AND. SF2->F2_TIPO $ 'ND' //Nota de Transfer ncia , avisa vendedores da entrada dos produtos na filial
...
nValmerc := SF2->F2_VALMERC //VALOR TOTAL DAS MERCADORIAS COM ACRESCIMMO DA CONDI O DE PAGAMENTO
...
///
nIdade2 := 0 //VARIAVEL PADR O PARA IDADE2
...
// 
// CUSTOS (OBS: qualquer altera o aqui, fa a tamb m no TMKVFIM 
// 
...
//Custo Margem de Contribui o Reposicao
...
// Valor TOTAL DO ITEM com acr scimo e sem IPI = BASE DE ICM (UB_VRCACRE * UB_QUANT
...
// ou seja, a diferen a das despesas financeiras que pago para o banco das que o cliente me paga
...
//	IF (cUFOri $ 'MT') // Se for estado de GO ou MT o icm zerado
//	nValICM := 0
//	ELSE
...
//ENDIF
//Novo c lculo do PIS desprezando o Frete
...
// ********** COMISSOES ***********************************
// acrescer 63% ao valor do vendedor interno (comis1) em fun o de incid ncia de encargos trabalhistas
// descimo ter eiro, f rias , etc ----> marcio
...
// nTotItemSemIPI := NoRound(SD2->D2_PRCVEN * SD2->D2_QUANT + (SC6->C6_VALOR / SUA->UA_VALMERC * SUA->UA_DESPESA ),2) + nICMRetido + nValFrtA ///// Alterado por rodrigo para contemplar as novas regras de calculo, por m considerando o frete 11/06/2015
...
//ENDIF
ELSE
// nTotItemSemIPI := NoRound(SD2->D2_PRCVEN * SD2->D2_QUANT + (SC6->C6_VALOR / SUA->UA_VALMERC * SUA->UA_DESPESA ),2) + nICMRetido + nValFret	///// Alterado por rodrigo para contemplar as novas regras de calculo, por m considerando o frete 11/06/2015
...
// Margem de Contribuicao n o Gerencial (sem os fatores -> F1, F2, F3 ...)
...
//nVDescComis := nQtdItem * nVlrItem * ( (n_COMIS1 * 1.63) + n_COMIS2 + (n_COMIS3 * 1.63) + (n_COMIS4 * 1.63) + (n_COMIS5 * 1.63) )/100 // alterado por rodrigo soluto em 25/05/2015 em fun o de que deve retornar a incid ncia de 63% de encargos trabalhistas no calculo de comiss es.
...
// Incluir o percentual de 8,25% no custo reposi o nas vendas que se enquadram na substitui o tribut ria para as filiais: RS , MT, PR, MG.
...
//Indices da Margem de Contribui o (DIVIDIR PELO VALOR TOTAL DA NOTA sem IPI, ou seja igual a base de ICM )
// N o Gerencial
...
If SF4->F4_DUPLIC='S'//Edivaldo Gon alves Cordeiro
...
Public cUFOri := " "
//cFilAnt -> variavel publica do sistema,	N mero da Filial que est em uso no momento.
...
//u_Mensagem("O desconto de ICMS padr o para vendas com origem em "+cOrigem+" e destino "+cDestino+" n o est cadastrado.")
Final("Erro ao gravar pedido de venda.")

ConOut("O desconto de ICMS padr o para vendas com origem em "+cOrigem+" e destino "+cDestino+" n o est cadastrado.")
...
//|Autor.....: Edivaldo Gon alves Cordeiro
//|Data......: 13/05/2010
//|Descricao.: Atualiza comiss o por Operador na grava o da Nota Fiscal
//|Observa o:
//+-----------------------------------------------------------------------------------//
*--------------------------------------------------*
...
If Empty(SUA->UA_OPERAD2) .OR. SUA->UA_OPERAD2=SUA->UA_OPERADO .OR. SA1->A1_GERVEN <> '000685' //-------N o h dois operadores,portando n o precisa gerar a segunda comiss o.
Return
Else
...
//Identifico os percentuais de comiss o dos vendedores
...
//Calcula Diferen a da Comiss o entre o Cadastro(SA3) e Comiss o no T tulo SE3
nDifComis := nE3PORC / nComVend1
nPerComVenda := nComVend2 *nDifComis



cSql :=" UPDATE "+RetSqlName("SE3")//--------------Atualiza a Base e o valor da Comiss o para os vendedores
cSql +=" SET "
...
cSql :=" UPDATE "+RetSqlName("SE3")//--------Atualiza o Percentual e valor da comiss o para o vendedor2
cSql +=" SET "
cSql +=" E3_BASE = " + STR(SE3->E3_BASE /2 ) + " , "
...
// Esse erro est ocorrendo no P8. No P10 n o ocorre.
// Essa fun o dever ser desativado no P10.
*****************************************************************************************
...
//|Descricao.: Retorna o percentual de comiss o para o 2 Operador do Call Center
//|Uso.......: U_SendNfTrf
//|Observa o: Envia o e-mail dos Boletins de Entrada
//+-----------------------------------------------------------------------------------//
...
//|Descricao.: Carrega os itens de transfer ncia para envio do Boletim de Entrada
//|Uso.......: U_SendNfTrf
//|Observa o: Envia o e-mail dos Boletins de Entrada
//+-----------------------------------------------------------------------------------//
