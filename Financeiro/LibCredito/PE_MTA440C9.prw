...
If ! SC5->C5_TIPO $"C/I/P" .AND. SF4->F4_ESTOQUE == 'S' //.AND.  FUNNAME()='MATA410' //Se movimentar Estoque verifico se trata Controle de Localização
	...
	If lRetWMS
        If Empty(SC9->C9_ENDPAD) .Or. Empty(SC9->C9_SERVIC) .Or. SC9->C9_BLWMS <> "01"
			SC9->C9_ENDPAD :=SF4->(Posicione("SBE",6,xFilial("SBE") +'000007'+'01',"BE_LOCALIZ"))
			SC9->C9_SERVIC :=SF4->(Posicione("DC5",2,xFilial("DC5") +'002',"DC5_SERVIC"))
...
U_ExecMySql(" DELETE SC9010 WHERE  R_E_C_N_O_ = '"+Alltrim(Str(nSC9NewRec))+"' " ,"" ,"E" )
