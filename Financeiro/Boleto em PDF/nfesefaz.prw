...
///JULIO JACOVENKO, em 18/12/2014
///ajuste ref Local de Entrega quando
///nf entrada tipo B ou D, n„o È para
///aparecer


/////JULIO JACOVENKO, em 10/12/2014
/////ajustes no aDest[14] e aDest[13]
/////estava aparecendo inscr


/////JULIO JACOVENKO, em 08/12/2014
/////AJUSTES REF BUG NA ROTINA RERPRVUNIT
/////SUBSTITUIDA PELOS CAMPOS CORRETOS...
/////
/////AJUSTADO CASAS DECIMAIS DAS TAG VUNCOM e VUNTRIB para 4
/////de acordo com o bando de dados.
/////para evitar erros de arrendondamento no SEFAZ

////JULIO JACOVENKO, em 11/11/2014
////feita a validaÁ„o para entrada de notas de importaÁ„o
////conf. email da Adriana no dia de hoje.

///JULIO JACOVENKO, em 12/11/2014
///Corrigido o peso bruto e peso liquido da nota de saida

///JULIO JACOVENKO, em 30/09/2014
///ajustado para Filiais
///04 - Goiania - questao descontos sefaz aparecer somente nas
///     informaÁoes complementares
...
±±≥          ≥Ajustado por Julio Jacovenko em 22/11/2012                  ≥±±
±±≥          ≥para usO da IMDEPA                                          ≥±±
±±≥          ≥SA1->A1_MAILNFE                                             ≥±±
...
//ALERT('...NFESEFAZ...SENDO TRABALHADO...PODE CONTINUAR...')
	...
	If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
	   ...
	   U_FATUPESO(cNOTA,cSERIE,'')
		...
		ElseIf FunName() == "SPEDNFSE"
			...
			/*
			If "/" $ FisGetEnd(ALLTRIM(SA1->A1_TLOGEND)+' '+SA1->A1_END+' '+ALLTRIM(SA1->A1_COMPEND),SA1->A1_EST)[3]
				aadd(aDest,IIF(FisGetEnd(ALLTRIM(SA1->A1_TLOGEND)+' '+SA1->A1_END+' '+ALLTRIM(SA1->A1_COMPEND),SA1->A1_EST)[3]<>"",FisGetEnd(ALLTRIM(SA1->A1_TLOGEND)+' '+SA1->A1_END+' '+ALLTRIM(SA1->A1_COMPEND),SA1->A1_EST)[3],"SN"))
			Else
 				aadd(aDest,IIF(FisGetEnd(ALLTRIM(SA1->A1_TLOGEND)+' '+SA1->A1_END+' '+ALLTRIM(SA1->A1_COMPEND),SA1->A1_EST)[2]<>0,FisGetEnd(ALLTRIM(SA1->A1_TLOGEND)+' '+SA1->A1_END+' '+ALLTRIM(SA1->A1_COMPEND),SA1->A1_EST)[2],"SN"))
			EndIf
			*/


            //JULIO JACOVENKO, em 01/04/2014
            //Ajustado para endereco
            aadd(aDest,FisGetEnd(ALLTRIM(SA1->A1_TLOGEND)+' '+AllTrim(U_CorEnd(SA1->A1_END))+' '+ALLTRIM(SA1->A1_COMPEND),SA1->A1_EST)[4])

			//aadd(aDest,FisGetEnd(ALLTRIM(SA1->A1_TLOGEND)+' '+SA1->A1_END+' '+ALLTRIM(SA1->A1_COMPEND),SA1->A1_EST)[4])
		...
		Else
			...
			If !SF2->F2_TIPO $ "DB"
				...
				If MyGetEnd(ALLTRIM(SA1->A1_TLOGEND)+' '+SA1->A1_END+' '+ALLTRIM(SA1->A1_COMPEND),"SA1")[2]<>0
					aadd(aDest,MyGetEnd(SA1->A1_END,"SA1")[3])
				Else
					aadd(aDest,"SN")
				EndIf

				aadd(aDest,IIF(SA1->(FieldPos("A1_COMPLEM")) > 0 .And. !Empty(SA1->A1_COMPLEM),SA1->A1_COMPLEM,MyGetEnd(SA1->A1_TLOGEND+' '+SA1->A1_END+' '+SA1->A1_COMPEND,"SA1")[4]))
				...
				///////JULIO JACOVENKO, em 19/12/2013
				///////REF CODIGO DO PAIS
				///////CRISTIANO ALIMENTOU A TABELA SYA COMO OS DADOS DA TABELA CCH
				///////
				///////
				aadd(aDest,IIF(Empty(SA1->A1_PAIS),"1058"  ,Posicione("SYA",1,xFilial("SYA")+SA1->A1_PAIS,"YA_SISEXP")))    ///A1_CODPAIS
				//PARA USAR DA TABELA DE CLIENTES
				//aadd(aDest,IIF(Empty(SA1->A1_PAIS),"1058"  ,SA1->A1_CODPAIS)))
            ...
            aadd(aDest,cEMAILCLI)
				...
				If SF2->(FieldPos("F2_CLIENT"))<>0 .And. !Empty(SF2->F2_CLIENT+SF2->F2_LOJENT) .And. SF2->F2_CLIENT+SF2->F2_LOJENT<>SF2->F2_CLIENTE+SF2->F2_LOJA
					...
					aadd(aEntrega,MyGetEnd(ALLTRIM(SA1->A1_TLOGEND)+' '+SA1->A1_END+' '+ALLTRIM(SA1->A1_COMPEND),"SA1")[1])
					aadd(aEntrega,ConvType(IIF(MyGetEnd(ALLTRIM(SA1->A1_TLOGEND)+' '+SA1->A1_END+' '+ALLTRIM(SA1->A1_COMPEND),"SA1")[2]<>0,MyGetEnd(ALLTRIM(SA1->A1_TLOGEND)+' '+SA1->A1_END+' '+ALLTRIM(SA1->A1_COMPEND),"SA1")[2],"SN")))
					aadd(aEntrega,MyGetEnd(ALLTRIM(SA1->A1_TLOGEND)+' '+SA1->A1_END+' '+ALLTRIM(SA1->A1_COMPEND),"SA1")[4])
			...
			While ( !Empty(cScan) )
				...
				If !Empty(cEspecie)
					...
					If ( nScan==0 )
					    ...
					    aadd(aEspVol,{ cEspecie, FieldGet(FieldPos("F2_VOLUME"+cScan)) , SF2->F2_PLIQUI , SF2->F2_PBRUTO,CMARCA,CNUMERACAO})
			...
			#IFDEF TOP
				...
				BeginSql Alias cAliasSD2
						SELECT D2_FILIAL,D2_SERIE,D2_DOC,D2_CLIENTE,D2_LOJA,D2_COD,D2_TES,D2_NFORI,D2_SERIORI,D2_ITEMORI,D2_TIPO,D2_VALICM,D2_PICM,D2_ITEM,D2_CF,
...
//////////////////////////////////////////////////////////////
				...
				If !Empty(SC5->C5_MENPAD) .And. !AllTrim(FORMULA(SC5->C5_MENPAD)) $ cMensFis
					...
					EndIf
					  ...
					  /*
					 ///JULIO JACOVENKO
					 ///ADICIONADO PARA COMPATIBILIZAR COM TELA DA VERDADE
					 ///a principio nao necessita destas linhas, j· que o
					 ///protheus trabalha na formulas do SM4...

		                cFormula:=(cAliasSD2)->(POSICIONE("SM4",1,XFILIAL("SM4")+ALLTRIM(CMENPAD),"M4_FORMULA"))


						If SM4->M4_CODIGO <> "210"
							cMensFis += AllTrim(FORMULA(SC5->C5_MENPAD))
						Else

							cTEXTO := AllTrim("ICMS DIfERIDO EM 33,33% CFE ART 96,DECRETo 1980/07-RICMS/PR("+u_MICMDIf(SF2->F2_FILIAL,SF2->F2_DOC,SF2->F2_SERIE)+")")
				            //conteudo M4_CODIGO='210' //"ICMS DIFERIDO EM 33,33% CFE ART 96, DECRETO 1980/07-RICMS/PR "+u_MICMDIF(SF2->F2_FILIAL,SF2->F2_DOC)
							cMensFis += AllTrim(cTEXTO)
			...
			//+---------------------------------------------------------------------------------------------------------//
			//|ProjeTo LÛgico Nfe_PreÁo de CusTo - TES 720 Vendas de Mercadorias sujeitas ao regime ST dentro do Estado
			//|Inserido por Edivaldo GonÁalves Cordeiro em 28/04/2010
			//+---------------------------------------------------------------------------------------------------------//
			nBaseIcms:=0
			nBaseIcmsTot:=0
			nIcmsRetTot:=0
		...
		//    Endif
			...
			/////////////////////////////////////////////////////////////////////
				...
				If !EMPTY(_cCusind) .AND. SF2->F2_FILIAL = "13"
				   ...
				   IF !AllTrim(" * Base de calculo e valor do ICMS ST, cfe Art 4 do Anexo X do Decreto 6.080/12 - RICMS/PR* ") $ cMensFis
				     cMensFis += " * Base de calculo e valor do ICMS ST, cfe Art 4 do Anexo X do Decreto 6.080/12 - RICMS/PR* "
				   ENDIF
				EndIf
		       ...
		       If ((SF2->F2_FILIAL = "04" .AND. SD2->D2_TES = "721" ) .OR. (cImdepa = SF2->F2_CLIENTE .AND. SF2->F2_EST = "GO" .AND. SD2->D2_TES = "653");  // Agostinho - 24/10/2011 - Chamados AAZUEY e AAZUET da Adriana
			      .OR. (cImdepa = SF2->F2_CLIENTE .AND. SF2->F2_EST = "GO" .AND. SD2->D2_CF = "6152" .AND. SF4->F4_ESTOQUE = "N"))  // Agostinho - 20/10/2011 - Chamado AAZUTH da Adriana
			    ...
			    If ( (SF2->F2_FILIAL = "09" .AND. SD2->D2_TES $ "653/661" .AND. SF2->F2_EST = "MA") .OR. (SF2->F2_FILIAL <> "11" .AND. SD2->D2_TES $ "653" .AND. SF2->F2_EST = "MA") )
			   ...
			   If SF2->F2_FILIAL <> "06" .AND. SD2->D2_TES = "653" .AND. SF2->F2_EST = "SC"
			...
			///////////////////////////////////////////////TRATAMENTO CODIGO, DESC. PRODUTO E MENSAGENS IMDEPA
                ...
                //(((cAliasSD2)->D2_TOTAL + nDesconto) /(cAliasSD2)->D2_QUANT),;
                //RetPrvUnit(cAliasSD2,nDesconto),; //NAO HE MAIS NECESSARIO  aprod[16]
				aadd(aProd,	{Len(aProd)+1,;
					cCodProd,;
					IIf(Val(SB1->B1_CODBAR)==0,"0",StrZero(Val(SB1->B1_CODBAR),Len(Alltrim(SB1->B1_CODBAR)),0)),;
					cDescProd,;
				...
				//JULIO JACOVENKO, em 05/09/2013
				//ajustado para incluir nas taq do xml o pedido de compra e item
				//DbSelectArea("SC6")
				//DbSetOrder(1)
				If SC6->(MsSeek(xFilial("SC6")+(cAliasSD2)->D2_PEDIDO+(cAliasSD2)->D2_ITEMPV))
				...
				ElseiF AliasIndic("CDL")
					...
					ElseIf SA1->A1_EST='EX'
				    	aAdd(aDados,{"ZA02","ufEmbarq"  , AllTrim(SM0->M0_ESTENT) })
				    	aAdd(aDados,{"ZA03","xLocEmbarq", AllTrim(SM0->M0_CIDENT) })
				...
				///JULIO JACOVENKO, em 14/05/2014
				///PARA QUANDO EXPORTACAO, aqui feito para compatibilizar
				///com o que e feito para uni5. J· que o PROTHEUS usa  a tabela CDL - Complemento de Exportacao
				///e na Imdepa n„o estamos usando esta tabela.
				///Previs„o: em algum momento vamos ter de usar estas tabelas.
				ElseIf SA1->A1_EST='EX'
				    	aAdd(aDados,{"ZA02","ufEmbarq"  , AllTrim(SM0->M0_ESTENT) })
				    	aAdd(aDados,{"ZA03","LocEmbarq", AllTrim(SM0->M0_CIDENT) })
				    	aAdd(aExp,aDados)
				Else

					//cXml += '<UFEmbarq>'	+AllTrim(SM0->M0_ESTENT)	+ '</UFEmbarq>' 	+ENTER
	                //cXml += '<xLocEmbarq>'	+AllTrim(SM0->M0_CIDENT)	+ '</xLocEmbarq>'	+ENTER
				...
				If AliasIndic("CD6")  .And. CD6->(FieldPos("CD6_QTAMB")) > 0 .And. CD6->(FieldPos("CD6_UFCONS")) > 0  .And. CD6->(FieldPos("CD6_BCCIDE")) > 0 .And. CD6->(FieldPos("CD6_VALIQ")) > 0 .And. CD6->(FieldPos("CD6_VCIDE")) > 0
					...
					IF ALLTRIM((cAliasSD2)->D2_CF) $ ALLTRIM(ALLTRIM(GETMV("MV_CFGRAX1")) + ALLTRIM(GETMV("MV_CFGRAX2"))) // Tag das Graxas
					   aadd(aComb,{ALLTRIM(GETMV("MV_ANPGRAX")),"",(cAliasSD2)->D2_QUANT,Iif(SF2->F2_TIPO $ "DB" ,SA1->A1_EST,SA2->A2_EST)})
					ELSE
					   aadd(aComb,{})
					ENDIF

					//aadd(aComb,{"650101001","",(cAliasSD2)->D2_QUANT,Iif(!SF2->F2_TIPO $ "DB",SA1->A1_EST,SA2->A2_EST)})
				...
				*/
				...
				//JULIO JACOVENKO, em 14/05/2014 - colocado direto para pegar F2_EST
				//ajustado para combustiveis....
				...
				While CD2->(!Eof()) .And. xFilial("CD2") == CD2->CD2_FILIAL .And.;
					...
					///CD2->CD2_ORIGEM
					///IF(ALLTRIM(CD2->CD2_ORIGEM)='',0,CD2->CD2_ORIGEM)

					Do Case
						Case AllTrim(CD2->CD2_IMP) == "ICM"
							aTail(aICMS) := {IF(ALLTRIM(CD2->CD2_ORIGEM)='','0',CD2->CD2_ORIGEM),;
											   If(lNfCupZero,POSICIONE("SF4",1,xFilial("SF4")+cD2Tes,"F4_SITTRIB"),CD2->CD2_CST),;
											   CD2->CD2_MODBC,;
							                   If(lNfCupZero,0,nMargem),;
											   If(lNfCupZero,0,FICMzero((cAliasSD2)->D2_TIPO, (cAliasSD2)->D2_VALICM, (cAliasSD2)->D2_PICM)),;                      //CD2->CD2_BC),;
							If(lNfCupZero,0,Iif(FICMzero((cAliasSD2)->D2_TIPO, (cAliasSD2)->D2_VALICM, (cAliasSD2)->D2_PICM)>0,CD2->CD2_ALIQ,0)),;
				...
				If lCalSol .OR.  lMVCOMPET
					...
					If MsSeek(xFilial("SF3")+SF2->F2_CLIENTE+SF2->F2_LOJA+SF2->F2_DOC+SF2->F2_SERIE)
						If At (SF3->F3_ESTADO, cMVSUBTRIB)>0
                            ...
                            SubStr (cMVSUBTRIB, nPosI, nPosF)	//01 - IE_ST
						EndIf
					EndIf
			    Endif

				//JULIO JACOVENKO, em 20/01/2014
				//acima padr„o do Protheus, usando o MV_SUBTRI (que nem temos na producao), que n„o estmaos usando
			...
			// Msg Zona Franca de Manaus / ALC
				...
				If MsSeek(xFilial("SF3")+SF2->F2_CLIENTE+SF2->F2_LOJA+SF2->F2_DOC+SF2->F2_SERIE)
					If !SF3->F3_DESCZFR == 0
						...
						/////////////////////////////////////////////////////////////////////
						////JULIO JACOVENKO, em 29/09/2014
						////mostrar o percentual do ZONA FRANCA
						//NPICMZF:=((cAliasSD2)->D2_PICM)
						//str(NPICMZF,5,2)
						/////////////////////////////////////////////////////////////////////

						If lInfAdZF .And. (nValPisZF > 0 .Or. nValCofZF > 0)
							cMensFis += "Descs. Ref. a Zona Franca de Manaus/ALC. Desc Sefaz:"+str(NPICMZF,5,2)+" % ICMS - R$ "+str(SF3->F3_VALOBSE,13,2)+", PIS - R$ "+ str(nValPisZF,13,2) +"e COFINS - R$ " +str(nValCofZF,13,2)
							//str(SF3->F3_VALOBSE-SF2->F2_DESCONT-nValPisZF-nValCofZF,13,2)+", PIS - R$ "+ str(nValPisZF,13,2) +"e COFINS - R$ " +str(nValCofZF,13,2)
						ElseIF !lInfAdZF .And. (nValPisZF > 0 .Or. nValCofZF > 0)
							cMensFis += "Desc. Ref. ao ICMS - Zona Franca de Manaus/ALC. Desc Sefaz:"+str(NPICMZF,5,2)+" % R$ "+str(SF3->F3_VALOBSE,13,2)  //str(SF3->F3_VALOBSE-SF2->F2_DESCONT-nValPisZF-nValCofZF,13,2)
					    Else
					    	cMensFis += "Total do desconto Ref. a Zona Franca de Manaus/ALC. Desc Sefaz:"+str(NPICMZF,5,2)+" % R$ "+str(nValBse-SF2->F2_DESCONT,13,2)
					    EndIF
					EndIf
				EndIf
			EndIF
...
If lImporta
		...
		If SF1->F1_TIPO = 'N'
			...
			// incluÌdo por Luciano CorrÍa em 16/01/06...
			SW9->( dbSeTorder( 3 ) )
	...
	/////////////////////////////////////
		...
		Else
			...
			If SF1->F1_TIPO $ "DB"
				...
				aadd(aDest,MyGetEnd(ALLTRIM(SA1->A1_TLOGEND)+' '+SA1->A1_END+' '+ALLTRIM(SA1->A1_COMPEND),"SA1")[1])
		   		...
		   		If MyGetEnd(SA1->A1_END,"SA1")[2]<>0
					aadd(aDest,MyGetEnd(ALLTRIM(SA1->A1_TLOGEND)+' '+SA1->A1_END+' '+ALLTRIM(SA1->A1_COMPEND),"SA1")[3])
				Else
					aadd(aDest,"SN")
				EndIf
				...
				aadd(aDest,IIF(Empty(SA1->A1_PAIS),"BRASIL",Posicione("SYA",1,xFilial("SYA")+SA1->A1_PAIS,"YA_DESCR" )))
				aadd(aDest,SA1->A1_DDD+SA1->A1_TEL) //ADEST 13
				If !Upper(SA1->A1_EST) == "EX"
					aadd(aDest,VldIE(SA1->A1_INSCR))
				Else
					aadd(aDest,"")
			...
			Else
				...
				aadd(aDest,MyGetEnd(SA2->A2_END,"SA2")[4])  //ADEST 5

				If !Upper(SA2->A2_EST) == "EX"
				   aadd(aDest,SA2->A2_BAIRRO)
				ELSE
                   aadd(aDest,'N=A')	 //ADEST 6
				ENDIF
				...
				Else
				   ...
				   ///ajuste importacao cliente estrategico
		             ...
		             EndIf
				    ////////////////////////////////////////////////////////////
					aadd(aDest,"99999")			  //ADEST 7
					aadd(aDest,"EXTERIOR")        //ADEST 8
				...
				aadd(aDest,Upper(SA2->A2_EST))     //ADEST 9
				aadd(aDest,SA2->A2_CEP)            ///ADEST 10
				...
				ELSE
				  ...
				  aadd(aDest,IIF(Empty(SA2->A2_PAIS),"1058"  ,Posicione("SYA",1,xFilial("SYA")+SA2->A2_PAIS,"YA_SISEXP")))   //ADEST 11
				  aadd(aDest,IIF(Empty(SA2->A2_PAIS),"BRASIL",Posicione("SYA",1,xFilial("SYA")+SA2->A2_PAIS,"YA_DESCR")))    //ADEST 12

				ENDIF

				aadd(aDest,SA2->A2_DDD+SA2->A2_TEL) //ADEST 13

				If !Upper(SA2->A2_EST) == "EX"
					aadd(aDest,VldIE(SA2->A2_INSCR))      //ADEST 14
				Else
					aadd(aDest,"")
				EndIf

				aadd(aDest,"")  //SA2->A2_SUFRAMA   //ADEST 15
				aadd(aDest,SA2->A2_EMAIL)           //ADEST 16
				aAdd(aDest, "") // PosiÁ„o 17 (referente a A1_CONTRIB, sendo passado como vazio j· que n„o existe A2_CONTRIB)

		        ////JULIO JACOVENKO, em 16/05/2014
		        ////ajuste para nota de importacao

		        _ForEST 	:= 	'N'
		        If SA2->A2_EST == 'EX'
		           _ForEST 	:= 	'F' //| Cliente Exporta
		           If LEFT(SA2->A2_COD,1) = "E"
		                _minharea:=getarea('SA2')
		                dBSelectArea('SZZ')
		                dBSetOrder(1)
	            ...
	            If _ForEST == "E"
	                //aDest-2
       				aadd(aDest,AllTrim(SZZ->ZZ_NOME))
       				//aDest-3
       				aadd(aDest,AllTrim(SZZ->ZZ_TLOGEND)+' '+AllTrim(SZZ->ZZ_END))
       			EndIf

       			//aDest-4
				If !Empty(SZZ->ZZ_NR_END)
					aadd(aDest,AllTrim(SZZ->ZZ_NR_END))
				Else
					aadd(aDest,"SN")
				EndIf

                //aDest-5
                If !empty(SZZ->ZZ_COMPEND)
			        aadd(aDest,"")
         		EndIf

                //aDest-6
				If !EMPTY(SZZ->ZZ_BAIRRO) .OR. !EMPTY(SZZ->ZZ_MUN)
				    aadd(aDest,AllTrim(If(!empty(SZZ->ZZ_BAIRRO),SZZ->ZZ_BAIRRO,SZZ->ZZ_MUN)))
				else
				    aadd(aDest,"")
				EndIf
			...
			If MsSeek(xFilial("SF3")+SF1->F1_FORNECE+SF1->F1_LOJA+SF1->F1_DOC+SF1->F1_SERIE)
				...
				////JULIO JACOVENKO, em 08/05/2014
				////
				////AQUI SOMO OS VALORES DO IPI
				////SE FOR O CASA DE TER MAIS DE UMA LINHA
				///FAZ UM WHILE
				While !SF3->(Eof()) .And. cClieFor+cLoja+cNota+cSerie == SF3->F3_CLIEFOR+SF3->F3_LOJA+SF3->F3_NFISCAL+SF3->F3_SERIE
				    IF SF3->F3_VALIPI>0 .AND. SF3->F3_TIPO=="D"
				     NTOTIPI+=SF3->F3_VALIPI
				    ENDIF
				    SF3->(DBSKIP())
				EndDo
				//////
            ...
            /*
            ///JULIO JACOVENKO, em 04/04/2014
            ///TRATAMENTO DAS MENSAGENS NOTA DE ENTRADA
            ///////////////////////////////////////////////////////
			If SM4->M4_CODIGO <> "210"
				cMensFis += AllTrim(SubStr(SM4->M4_ForMULA,2,Len(AllTrim(SM4->M4_ForMULA))-2)) + ' '
			Else
				cMensFis += Formula("210") +' '
			EndIf
			//////////////////////////////////////////////////////
	        //////////////////////////////////////////////////////
            */
			...
			//SF3->F3_VALIPI > 0 .And. SF3->F3_TIPO == "D"  .OR.
			If  NTOTIPI > 0
				If Len(cMensCli) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
					cMensFis += " "
				EndIf
				//cMensCli += "Valor do IPI: R$ " + AllTrim(Transform(SF3->F3_VALIPI, "@ze 9,999,999,999,999.99")) + " "
				cMensCli += "Valor do IPI: R$ " + AllTrim(Transform(NTOTIPI, "@ze 9,999,999,999,999.99")) + " "
			EndIf
			...
			#IFDEF TOP
				...
				BeginSql Alias cAliasSD1
				        ////JULIO JACOVENKO, 19/03/2013
				        ////incluido o campo D1_TESIMP para quando entrada importacao
						SELECT D1_FILIAL,D1_DOC,D1_SERIE,D1_FORNECE,D1_LOJA,D1_COD,D1_ITEM,D1_TES,D1_TESIMP,D1_TIPO,D1_NFORI,D1_SERIORI,D1_ITEMORI,
						...
						D1_NUMLOTE,D1_CUSTO,D1_ORIGLAN,D1_DESCICM,D1_CIF,D1_II,D1_FORMUL,D1_VALPS3,D1_ORIGLAN,D1_VALCF3 %Exp:cField%
						FROM %Table:SD1% SD1
						WHERE
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				Else
					...
					If Empty(cNatOper)
						//cNatOper := AllTrim(SubStr(SX5->X5_DESCRI,1,55))
						cNatOper := AllTrim(SX5->X5_DESCRI)
					Else
						cNatOper += Iif(!AllTrim(SubStr(SX5->X5_DESCRI,1,55)) $ cNatOper, "/ " + AllTrim(SubStr(SX5->X5_DESCRI,1,55)), "")
	    			EndIf
	    		EndIf
				...
				//aAdd(aInfoItem,{(cAliasSD1)->D1_PEDIDO,(cAliasSD1)->D1_ITEMPC,AllTrim(If(lImporta,(cAliasSD1)->D1_TESIMP,(cAliasSD1)->D1_TES)),(cAliasSD1)->D1_ITEM})
                ...
                ///JULIO JACOVENKO, em 19/11/2013
				///ajustar B2_CODBAR,para NAO COLOCAR BRANCO
...
///VUNCOM  APROD 16
//IIF(!(cAliasSD1)->D1_TIPO$"IP",IIF(!(SubStr((cAliasSD1)->D1_CF,1,1) $ "3"),(cAliasSD1)->D1_VUNIT,((cAliasSD1)->D1_VUNIT)),0),
				...
				aadd(aProd,	{Len(aProd)+1,;
					...
					SB1->B1_UM,;
					(cAliasSD1)->D1_QUANT,;
					IIF(!(cAliasSD1)->D1_TIPO$"IP",IIF(!(SubStr((cAliasSD1)->D1_CF,1,1) $ "3"),(cAliasSD1)->D1_TOTAL,(cAliasSD1)->D1_II + (cAliasSD1)->D1_CIF),0),;
					IIF(Empty(SB5->B5_UMDIPI),SB1->B1_UM,SB5->B5_UMDIPI),;
				...
				////JULIO JACOVENKO, 14/05/2014
				////LEARY HE VERDADEIRO, TEMO INTREGRACAO COM ESAY
				...
				If AliasIndic("CD6")  .And. CD6->(FieldPos("CD6_QTAMB")) > 0 .And. CD6->(FieldPos("CD6_UFCONS")) > 0  .And. CD6->(FieldPos("CD6_BCCIDE")) > 0 .And. CD6->(FieldPos("CD6_VALIQ")) > 0 .And. CD6->(FieldPos("CD6_VCIDE")) > 0
					//aadd(aComb,{CD6->CD6_CODANP,CD6->CD6_SEFAZ,CD6->CD6_QTAMB,CD6->CD6_UFCONS,CD6->CD6_BCCIDE,CD6->CD6_VALIQ, CD6->CD6_VCIDE })
				    ///JULIO 22/11/2012
				    ///aqui coloca o codigo ANP
					IF ALLTRIM(SD1->D1_CF) $ ALLTRIM(ALLTRIM(GETMV("MV_CFGRAX1")) + ALLTRIM(GETMV("MV_CFGRAX2"))) // Tag das Graxas
					   aadd(aComb,{ALLTRIM(GETMV("MV_ANPGRAX")),"",(cAliasSD1)->D1_QUANT,Iif(SF1->F1_TIPO $ "DB" ,SA1->A1_EST,SA2->A2_EST)})
					ELSE
					   aadd(aComb,{})
					ENDIF
				    //aadd(aComb,{"650101001","",(cAliasSD1)->D1_QUANT,Iif(SF1->F1_TIPO $ "DB" ,SA1->A1_EST,SA2->A2_EST)})
			    Elseif AliasIndic("CD6")  .And. CD6->(FieldPos("CD6_QTAMB")) > 0 .And. CD6->(FieldPos("CD6_UFCONS")) > 0
			    	aadd(aComb,{CD6->CD6_CODANP,CD6->CD6_SEFAZ,CD6->CD6_QTAMB,CD6->CD6_UFCONS})
				Else
					aadd(aComb,{})
			    ...
			    */
                    ...
                    //ALERT(ALLTRIM((cAliasSD1)->D1_CF))
                    //ALERT(ALLTRIM(ALLTRIM(GETMV("MV_CFGRAX1")) + ALLTRIM(GETMV("MV_CFGRAX2"))))
					...
					iF ALLTRIM((cAliasSD2)->D2_CF) $ ALLTRIM(ALLTRIM(GETMV("MV_CFGRAX1")) + ALLTRIM(GETMV("MV_CFGRAX2"))) // Tag das Graxas
					   aadd(aComb,{ALLTRIM(GETMV("MV_ANPGRAX")),"",(cAliasSD2)->D2_QUANT,SF1->F1_EST})  //Iif(SF2->F2_TIPO $ "DB" ,SA1->A1_EST,SA2->A2_EST)})
					ELSE
					   aadd(aComb,{})
					ENDIF
...
IF !LIMPORTA
           ...
           cLocEnt:=''
	        ...
	        ////SOMENTE DESTACAR LOCAL DE ENTREGA QUANDO
	        ////FOR F2_TIPO DIF de D ou B
	          cLocEnt:=''
	        ENDIF
		...
		If ! Empty(SC5->C5_CLIENTR)
			...
			cMCli += ' Inscricao Estadual: '+SA1->A1_INSCR
		...
		/*
		If !EMPTY(_cCusind) .AND. SF2->F2_FILIAL = "09"
			cMenNfe += " * Base de c·lculo p/seu crÈdiTo ICMS/ST, cfe par·grafo 3∫, Art. 274, DecreTo 45.390/00 RICMS/SP" +ENTER
		EndIf
        */
ELSE
...
If !Empty(aNota)
	...
	cString += NfeInfAd(cMensCli,cMensFis,aPedido,aExp,cAnfavea,aMotivoCont,aNota,aNfVinc,aProd,aDI,aNfVincRur,aRetido)

	//cXml += " Codigo Imdepa do Fornecedor: ["+SF1->F1_ForNECE+"-"+SF1->F1_LOJA+"]"
...
////JULIO JACOVENKO, em 30/08/2013
////aqui entra a geracao do boleto (logo apos a geraÁ„o do XML)
////como he hoje.
////


// Movido para o danfeII.Prw
//ALERT('...GERA BOLETO...')
//U_IMDR120()  //FAZER AJUSTES PARA CONTINUAR USANDO O UNIFIVE


///AQUI CHAMA A FUNCAO PARA IMPRESSA DA DANFE.
///POIS TODA VEZ QUE TRANSMITE, CHAMA A IMPRESSAO DA DANFE....
///ACHO QUE TEM DE DAR UM 'TIME'....POIS LEVA UM 10 SEGUNDOS ATE LIBERAR
///u_PrtNfeSef(cIdEnt,cSERIE,cNOTA,oDanfe,oSetup,cFilePrint)
...
Static Function NfeDest(aDest,cVerAmb,aTransp,aCST)
    ...
    //ver este caso para cliente 005195 LOJA 4 PARANA estaca com NAO = 2
    //posicao 17 //SA1->A1_CONTRIB
...
///26/01/2012
NTAMEAN:=LEN(ALLTRIM(APROD[03])) //LEN(ALLTRIM(CONVTYPE(APROD[03])))
IF NTAMEAN<15
   CEAND:=STRZERO(VAL(ALLTRIM(APROD[03])),15)
   cString += '<ean>'+CEAND+'</ean>'
ELSE
...
Else
	cString += '<vUnCom>'+ConvType(aProd[16],16,8)+'</vUnCom>'
Endif
...
If Len(aDI)>0 .And. ConvType(aDI[04][1]) == "I19"
    ...
    ///JULIO JACOVENKO, em 19/03/2014
    ///alterado para atender importacao
	IF LIMPORTA

       iareaSF2:=SF2->(GetArea())
       iareaSD2:=SD2->(GetArea())

		SW6->( dbSeTorder(1) )
		SW6->( dbSeek(xFilial()+SF1->F1_HAWB) )

	   cString += '<LocDesemb>'+ALLTRIM(Posicione("SJ0",1,xFilial("SJ0")+SW6->W6_URF_DES,"J0_DESC"))+ '</LocDesemb>'
	   cString += '<UFDesemb>'+AllTrim(SW6->W6_UF_DES)+ '</UFDesemb>'  //AllTrim(SW6->W6_UF_DES)
...
////////////JULIO JACOVENKO, em 16/05/2014
////////////tratamento quando importacao - padr„o UNI5
//| Testa se eh nota de Importacao ou Nacional
If ! Empty(SF1->F1_HAWB)    // Nota de Importacao
	lImporta := .T.
Else
...
////////////////tratamento quando importacao - padrao UNI5//////////////////////////////////////
//////////////////////////////////////////////////////
...
If Len(aComb) > 0  .And. !Empty(aComb[01])
    ...
    //If Len(aComb) > 4 .and. !Empty(aComb[05])
	    	...
	    	cSTRING += '<CIDE>'
			cSTRING += '<qBCProd>0</qBCProd>'
			cString += '<vAliqProd>0</vAliqProd>'
			cString += '<vCIDE>0</vCIDE>'
			cString += '</CIDE>'
	...
	cString += '</comb>'
                          //JULIO COLOCOU O .AND.
ElseIf !Empty(aProd[17]) .And. !Empty(aComb[4])
	cString += '<comb>'
...
If  !lIssQn
	...
	ELse
		...
		If aCST[01] = "60" .AND. cTipo=="1"
           ...
           nBaseIcm :=  SB1->(U_nRetBaseRed( XFILIAL("SB1"), aProd[02])) // 23/01/2015 - Agostinho - Busca valor da ZA7
           nValICM  :=  SB1->(U_nRetIcmsRet( XFILIAL("SB1"), aProd[02])) // 23/01/2015 - Agostinho - Busca valor da ZA7
...
/*                                      //aqui deveria ser SA2
If ! (FConvChar(SM0->M0_CGC) = FConvChar(SA1->A1_CGC)) // Nota de ajuste
    cXML :=     REGTAGAA(cXml)             //| AA.00 - InFormacoes do transporte da NF-e
    cXML :=     REGTAGAB(cXml)            //≥ AB.00 - Dados da Cobranca
Else
    cXml += '<transp>'
    cXml += '<modFrete>0</modFrete>'
    cXml += '</transp>'
Endif
*/
...
If (FConvChar(SM0->M0_CGC) = FConvChar(SA2->A2_CGC)) // Nota de ajuste
    cModFrete:="0"
Endif
...
For nX := 1 To Len(aVol)
	cString += '<vol>'
		...
		cString += '<marca>' +ConvType(aVol[NX][05])+'</marca>'
		cString += '<nVol>'  +ConvType(aVol[NX][06])+'</nVol>'
...
Static Function NfeInfAd(cMsgCli,cMsgFis,aPedido,aExp,cAnfavea,aMotivoCont,aNfSa,aNfVinc,aProd,aDI,aNfVincRur,aRet)
...
////////////////////31/03/2014
	If lImporta
		...
		_cInfImp := ""
		If !EMPTY(cNumDI)
			_cInfImp += "NUMERO DA DI " + cNumDI
		EndIf

		If !EMPTY(cNumProc)
			_cInfImp += " NUMERO DO PROCESSO " + cNumProc
		EndIf

		If SF1->F1_TIPO <> "N" .and. !Empty(__cNForig)
			_cInfImp += " COMPLEMENTo DA NF " + __cNForig
		EndIf

		If !EMPTY(SF1->F1_DESPESA)
			_cInfImp += " DESPESA ADUANEIRA: " + STR(SF1->F1_DESPESA,12,2)
		EndIf

		/*
		If !EMPTY(nSG)
			_cInfImp += " SEGURO: " + STR(nSG,12,2)
		EndIf
		*/

		If !EMPTY(nFR)
			_cInfImp += " FRETE: " + STR(nFR,12,2)
		EndIf

		If !EMPTY(nII)
			_cInfImp += " IMPOSTo DE IMPORTACAO: " + STR(nII,12,2)
		EndIf


		//NVALPIS:=(cAliasSD1)->D1_VALIMP6
		If !EMPTY(nValPis)
			_cInfImp += " PIS: " + STR(nValPis,12,2)
		EndIf

	    //NVALCOF:=(cAliasSD1)->D1_VALIMP5
		If !EMPTY(nValPis)
			_cInfImp += " COFINS: " + STR(nValCof,12,2)
		EndIf
	...
	cString += " Codigo Imdepa do Fornecedor: ["+SF1->F1_ForNECE+"-"+SF1->F1_LOJA+"]"
/////////
...
Else
    ...
    ///pegava valor do prunit e n„o fechava com os outros valores...
...
/*‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±…ÕÕÕÕÕÕÕÕÕÕ—ÕÕÕÕÕÕÕÕÕÕÕÕÀÕÕÕÕÕÕÕ—ÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÀÕÕÕÕÕÕ—ÕÕÕÕÕÕÕÕÕÕÕÕÕª±±
±±∫Programa  ≥xClearAcentos∫Autor  ≥Cristiano Machado∫ Data ≥  07/08/10   ∫±±
±±ÃÕÕÕÕÕÕÕÕÕÕÿÕÕÕÕÕÕÕÕÕÕÕÕ ÕÕÕÕÕÕÕœÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕ ÕÕÕÕÕÕœÕÕÕÕÕÕÕÕÕÕÕÕÕπ±±
±±∫Desc.     ≥Retira acentos das strings enviadas a funcap               ∫±±
±±∫          ≥                                                           ∫±±
±±ÃÕÕÕÕÕÕÕÕÕÕÿÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕπ±±
±±∫Uso       ≥ AP                                                        ∫±±
±±»ÕÕÕÕÕÕÕÕÕÕœÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕº±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂ*/
**********************************************************************
User Function xClearAcentos(xTxt)
**********************************************************************
Local cAcentos  := "Á«Äá·ÈÌÛ˙¡…Õ”⁄‚ÍÓÙ˚¬ Œ‘€‡ÈÌÛ˙¡…Õ”⁄‰ÎÔˆ¸ƒÀœ÷‹„ı√’é"+chr(65533)+"¿≈ÖÜ†Ñ¶Â"+chr(65533)+"»àÇËÃ°"+chr(65533)+"“÷ìîï¢ßŸ£"+chr(65533)+"˘—Ò%$˛˜›∞∫™¨É&"
...
Return(nValnicm)
**************************************************************************
Static Function FConvChar(cTxt)//Usado em CNPJ e IE para retirar caracteres que n„o sejam numeros.
**************************************************************************
