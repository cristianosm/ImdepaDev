...
Private cTitAux := ""    // Guarda o titulo do relatÛrio para R3 e R4 
...
//> mv_par01	  // do Numero 			  >
//> mv_par02	  // atÇ o Numero 		  >
//> mv_par03	  // do Prefixo			  >
//> mv_par04	  // atÇ o Prefixo		  >
//> mv_par05	  // da Natureza  	     >
//> mv_par06	  // atÇ a Natureza		  >
//> mv_par07	  // do Vencimento		  >
//> mv_par08	  // atÇ o Vencimento	  >
//> mv_par09	  // do Banco			     >
//> mv_par10	  // atÇ o Banco		     >
//> mv_par11	  // do Fornecedor		  >
//> mv_par12	  // atÇ o Fornecedor	  >
//> mv_par13	  // Da EmissÑo			  >
//> mv_par14	  // Ate a EmissÑo		  >
//> mv_par15	  // qual Moeda			  >
...
If Empty(mv_par33)
    Help(" ",1,'FINDTBASE',,STR0066,1,0,,,,,,{STR0067} ) //"Data Base nao informada na parametrizacao do relatorio."  ###  "Por favor, informe a data base nos parÉmetros do relatorio (pergunte)."
    oReport:CancelPrint()
    Return
Endif
...
//*******************************************************
// SoluÁ„o para o problema no filtro de Serie minuscula *
//*******************************************************
...
//> Define as quebras da seÁ„o, conforme a ordem escolhida.      >
...
//			Store 0 To nFil0,nFil1,nFil2,nFil3,nFil4,nFilTit,nFilJ
			While &cCond2 == cCarAnt .and. !Eof() .and. lContinua .and. SE2->E2_FILIAL == xFilial("SE2")
				...
				// compatibilizando com a versÑo 2.04 que nÑo gerava para titulos
				// de ISS e FunRural
				...
				//> Verifica se esta dentro do parametro compor pela data de digitaÁ„o >
				...
				If dDataBase > SE2->E2_VENCREA 		//vencidos
					...
					If MV_PAR21 == 1 // se compoem saldo retroativo verifico se houve baixas
						If !Empty(dUltBaixa) .And. dDataBase < dUltBaixa
							dUltBaixa := FR150DBX() // Ultima baixa atÈ DataBase
						EndIf
					EndIf
...
//fffffffffffffffffffffffffffffffffffffff Localizaáîes ArgentinafY
If lFr150Flt
	cFr150Flt := EXECBLOCK("Fr150FLT",.f.,.f.)
Endif

///fffffffffffffffffffffffffffffffffffffffffffffffffffffffffff0
//> Variaveis utilizadas para ImpressÑo do Cabeáalho e RodapÇ >
			...
			While &cCond2 == cCarAnt .and. !Eof() .and. lContinua .and. E2_FILIAL == xFilial("SE2")
				...
				If mv_par21 == 1
					...
					If !Empty(xFilial("SE2")) .And. !Empty(xFilial("SE5"))
						nSaldo -= FRVlCompFil("P",SE2->E2_PREFIXO,SE2->E2_NUM,SE2->E2_PARCELA,SE2->E2_TIPO,SE2->E2_FORNECE,SE2->E2_LOJA,IIF(mv_par34 == 2,3,1),,,,mv_par15,SE2->E2_MOEDA,If(mv_par35==1,SE2->E2_TXMOEDA,Nil),dDataReaj,.T.)
					EndIf
					// Subtrai decrescimo para recompor o saldo na data escolhida - Checa se a SaldoTit j· considerou o DecrÈscimo.					
					If Str(SE2->E2_VALOR,17,2) == Str(nSaldo,17,2) .And. SE2->E2_DECRESC > 0 .And. SE2->E2_SDDECRE == 0
						nSAldo -= SE2->E2_DECRESC
					Endif
					// Soma Acrescimo para recompor o saldo na data escolhida - Checa se a SaldoTit j· considerou o AcrÈscimo.
					If Str(SE2->E2_VALOR,17,2) == Str(nSaldo,17,2) .And. SE2->E2_ACRESC > 0 .And. SE2->E2_SDACRES == 0
						nSAldo += SE2->E2_ACRESC
					Endif
				Else
...
//> ATENCAO !!!!                                               >
//> NÑo adiconar mais nada a chave do filtro pois a mesma est† >
//> com 254 caracteres.                                        >
...
If lExistFJU .and. !SX1->(dbSeek(PadR(cPerg,nTam)+"38"))
	...
	Aadd( aHelpPor, "Seleciona a opÁ„o (Sim) para que" )
	Aadd( aHelpPor, "seja considerado na posiÁ„o financeira os tÌtulos")
	Aadd( aHelpPor, "excluÌdos conforme DataBase." )
	Aadd( aHelpPor, " Seleciona a opÁ„o (N„o) ser· considerado " )
	Aadd( aHelpPor, "na posiÁ„o financeira os tÌtulos que n„o ")
	Aadd( aHelpPor, "foram deletados conforme DataBase. " )
	Aadd( aHelpSpa, "Seleccione opciÛn sino ser considerados" )
	...
	PutSx1(cPerg,"38","Considera Titulos Excluidos?" ,"?Considere TÌtulo Excluidos?" ,"Consider Title Excluded?","mv_chy","N",1,0,2,"C","","","","S","mv_par38","Sim","Si ","Yes","","Nao","No","No","","","","","","","","","")
...
Busca a data da ultima baixa realizada do titulo a pagar atÈ a
DataBase do sistema.
