...
////ajustado para o TIPO DE NOTA (B) Devolucao de Mercadoria que n„o venda (para uso)
...
///casos n„o sai o numero do cliente no
///endereco
///adest[04]
...
local cMVREFNFE		:= SuperGetMV("MV_REFNFE", ," ") 			// Parametro para informe quais CFOPs s„o de simples Remessa para levar informaÁ„o
...
Local cChaveD1		:= ""
Local aMensAux		:= {}
Local cMVAEHC 		:= AllTrim(GetNewPar("MV_AEHC",""))     // Informar o cÛdigo de classificaÁ„o AEHC
Local cHoraNota		:= ""
Local nA         	:= 0
Local cIndPres		:= ""
Local cIndIss		:= ""
Local cFilDev		:= ""		//Guarda filial de devoluÁ„o
Local cMsgDI		:= ""
Local nX         	:= 0
Local nY		 	:= 0
Local nCon       	:= 1
...
Local lCpoCusEnt	:= SF4->(FieldPos("F4_CUSENTR")) > 0 			//Tratamento para atender o DECRETO N∫ 35.679, de 13 de Outubro de 2010 - Pernambuco para o Ramo de Auto PeÁas
...
Private cIdRecopi	:= ""
Private cNumRecopi	:= ""
Private lCustoEntr := .F.	//Tratamento para atender o DECRETO N∫ 35.679, de 13 de Outubro de 2010 - Pernambuco para o Ramo de Auto PeÁas
Private cIdDest	:= ""
Private cIndFinal	:= ""
Private cIndIEDest := ""
Private cFntCtrb	:= ""
	   ...
	   //ALERT('...VAI CHAMAR A FUNCAO FATUPESO...')
		...
		Else
			...
			///cEMAILCLI:=U_xCLEARACENTOS(ALLTRIM(SA1->(Posicione("ZAY",1,XFILIAL('ZAY')+SA1->A1_COD+SA1->A1_LOJA,"ZAY_EMAIL"))) )
				...
				aadd(aDest,SA1->A1_PFISICA)//21-IdentificaÁ„o estrangeiro
			Else
			    dbSelectArea("SA2")
				dbSetOrder(1)
				...
				aadd(aDest,"")//PosiÁ„o 20
				aadd(aDest,SA2->A2_PFISICA)//21-IdentificaÁ„o estrangeiro
			EndIf
			//⁄ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒø
			...
			If SD2->(FieldPos("D2_GRPCST"))<>0 //Grupo de tributaÁ„o de ipi
			   cField  +=",D2_GRPCST"
			EndIf
			cField += "%"
			...
			While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				If lContinua
					...
					//Tratamento para atender o DECRETO N∫ 35.679, de 13 de Outubro de 2010 - Pernambuco para o Ramo de Auto PeÁas
								...
								dbSelectArea("SFT")
						   		...
						   		If MsSeek(xFilial("SFT")+"S"+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI)
									...
									ElseIf Alltrim(SFT->FT_CFOP) $ cMVREFNFE
										//Tratamento para que leve na TAG <refNFe> as notas referenciadas que contÈm o CFOP no par‚metro MV_REFNFE
										dbSelectArea("SF3")
								   		dbSetOrder(4)
...
//////////////////////////////////////////////////////////////
				...
				///cMENPAD:=SC5->C5_MENPAD
					...
					//Indicador de presenÁa do comprador no estabelecimento comercial no momento da operaÁ„o - VERS√O 3.10
					If SC5->(FieldPos("C5_INDPRES")) > 0
						 If lNfCup .Or. (cAliasSD2)->D2_ORIGLAN $ "VD|LO"
						 	cIndPres := "1" //1=OperaÁ„o presencial
						 Else
...
///////////////////////////////////////////////TRATAMENTO CODIGO, DESC. PRODUTO E MENSAGENS IMDEPA
                ...
                //IIF(!(cAliasSD2)->D2_TIPO$"IP",IIF(!(lMvNFLeiZF),(cAliasSD2)->D2_TOTAL+nDesconto+(cAliasSD2)->D2_DESCZFR,(cAliasSD2)->D2_TOTAL+nDesconto+(cAliasSD2)->D2_DESCZFR - ((cAliasSD2)->D2_DESCZFP+(cAliasSD2)->D2_DESCZFC)),IIF(((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. SubStr(SM0->M0_CODMUN,1,2) == "31") .Or. ((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. "RESSARCIMENTO" $ Upper(cNatOper) .And. "RESSARCIMENTO" $ Upper(cDescProd)),(cAliasSD2)->D2_TOTAL,0)),;
					aadd(aProd,	{Len(aProd)+1,;
						...
						cDescProd,;
						SB1->B1_POSIPI,;//Retirada validaÁ„o do parametro MV_CAPPROD, de acordo com a NT2014/004 n„o È mais possÌvel informar o capÌtulo do NCM
						SB1->B1_EX_NCM,;
						cD2Cfop,;
						SB1->B1_UM,;
						(cAliasSD2)->D2_QUANT,;
						...
						IIF((SF4->F4_AGREG='N' .And. !AllTrim(SF4->F4_CF) $ cMVCfopTran) .Or. (SF4->F4_ISS='S' .And. SF4->F4_ICM='N'),"0","1"),;// Tipo de agregaÁ„o de valor ao total do documento
					...
					If lEECFAT
						/*AlteraÁıes TQXWO2
						Na chamada da funÁ„o, foram criados dois novos par‚metros:
						o 3∫ referente ao cÛdigo do produto e o 4∫ referente ao n˙mero da nota fiscal + sÈrie (chave).
						GetNfeExp(pProcesso, pPedido, cProduto, cChave)
						No retorno da funÁ„o ser„o devolvidas as informaÁıes do legado, conforme leiaute anterior ‡ vers„o 3.10 ,
						e as informaÁıes dos grupos ìI03 - Produtos e ServiÁos / Grupo de ExportaÁ„oî e ìZA - InformaÁıes de ComÈrcio Exteriorî, conforme estrutura da NT20013.005_v1.21.
						As posiÁıes 1 e 2 mantÈm o retorno das informaÁıes ZA02 e ZA03, mantendo o legado para os cliente que utilizam vers„o 2.00
						Na posiÁ„o 3 passa a ser enviado o agrupamento do ID I50, tendo como filhos os IDs I51 e I52.
						Na posiÁ„o 4 passa a ser enviado o agrupamento do ZA01, tendo como filhos os IDs ZA02, ZA03 e ZA04.

						O array de retorno ser· multimensional, trazendo na primeira posiÁ„o o identificador (ID),
						na segunda posiÁ„o a tag (o campo) e na terceira posiÁ„o o conte˙do retornado do processo,
						...
						Do Case
							...
							Case AllTrim(CD2->CD2_IMP) == "ISS"
								...
								IF SF4->F4_ISSST == "1" .or. Empty(SF4->F4_ISSST)
									cIndIss := "1" //1-ExigÌvel;
								ElseIf SF4->F4_ISSST == "2"
									cIndIss := "2"	//2-N„o incidÍncia
	...
	/////////////////////////////////////
		...
		Else
			...
			If SF1->F1_TIPO $ "DB"
				...
				/* Quando houver uma troca/devoluÁ„o (LOJA720) de uma NFC-e no Estado do AM, a tag <infAdFisco>
				da NF-e de entrada deve conter o motivo de devoluÁ„o, nome, endereÁo e cpf do cliente
				O campo F1_MOTIVO È preenchido na funcao LOJA720 do SIGALOJA */
...
//aadd(aDest,SA1->A1_MAILNFE)
			...
			If SD1->(FieldPos("D1_DESCZFC")) > 0
				cField += ",D1_DESCZFC"
			EndIf
			If SD1->(FieldPos("D1_GRPCST"))<>0 //Grupo de tributaÁ„o de ipi
			   cField  +=",D1_GRPCST"
			EndIf
			cField += "%"
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				If !Empty((cAliasSD1)->D1_NFORI)
					...
					//Ajustes para que ao gerar nota de entrada do tipo complemento de preÁo de uma devoluÁ„o seja vinculado o cliente correto
					//da nota de origem.
					dbSelectArea("SD1")
					dbSetOrder(1)
		       ...
		       EndIf
				...
				//⁄ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒø
				//Tratamento para verificar se o produto È controlado por terceiros (IDENTB6)≥
...
*/
	...
	aadd(aProd,	{Len(aProd)+1,;
					...
					(cAliasSD1)->D1_DESPESA + (cAliasSD1)->D1_VALPS3 + (cAliasSD2)->D2_VALCF3 + nIcmsST,;//Outras despesas + PISST + COFINSST  (Inclus„o do valor de PIS ST e COFINS ST na tag vOutros - NT 2011/004).
					nRedBC,;//% ReduÁ„o da Base de C·lculo
					cCST,;//CÛd. SituaÁ„o Tribut·ria
					IIF(SF4->F4_AGREG<>'N' .And. SF4->F4_ISS='S',"1",IIF(SF4->F4_AGREG='N' .Or. (SF4->F4_ISS='S' .And. SF4->F4_ICM='N'),"0","1")),;// Tipo de agregaÁ„o de valor ao total do documento
					cInfAdic,;//Informacoes adicionais do produto(B5_DESCNFE)
					nDescZF,;
					(cAliasSD1)->D1_TES,;
					"",;
					0,;
					0,;  	// [30] Da posiÁ„o 28 a 30 tratamento realizado apenas para documento de saÌda por este motivo campos est„o zerados e vazios.
...
//Nota TÈcnica 2013/005 - Data e Hora no formato UTC
If cVeramb >= "3.10"
...
If cVeramb >= "3.10"
	...
	If SubStr(cCFOP,1,1) == "2" .Or. SubStr(cCFOP,1,1) == "6"
		 cOper:= "2" //OperaÁ„o Interestadual
	ElseIf SubStr(cCFOP,1,1) == "3" .Or. SubStr(cCFOP,1,1) == "7"
		cOper:= "3" //OperaÁ„o com Exterior
	Else
		cOper:= "1" //OperaÁ„o Interna CFOP 1 e 5
	EndIf

	//Identificador de Local de Destino da OperaÁ„o
...
/*Quando h· exportaÁ„o indireta(I52), deve-se informar as chaves(I54) na tag refNFe.
...
If !Empty(aExp[1]) .and. lEECFAT .and. cVerAmb >= "3.10"
	If Len(aExp) > 0 .and. aNota[04] == "1" //Somente se nota de saÌda
		For nX := 1 To Len(aExp)
			If Len(aExp[nX][3][3]) > 0
				For nY := 1 To Len(aExp[nX][3][3][2])
					//Quando n„o h· exportInd, a posiÁ„o 3 È retornada vazia
	...
	EndIf
/*SEM INTEGRA«√O COM EEC - Quando h· exportaÁ„o indireta, deve-se informar a chave(I54 - tag chNFe) na tag refNFe.
...
Do Case
	...
	Case (cVeramb >= "3.10" .and. (!Empty(aNfVinc) .Or. !Empty(aRefECF) .Or. !Empty(aNfVincRur))) .and. ( (aNota[5] $ "D|B" .And. SF4->F4_AJUSTE <> "S") .or. (aNota[5] $ "N" .And. SF4->F4_PODER3 == "D"))
   		...
   		If nPos > 0
   			cTPNota:= "1"
   		Else
   			cTPNota:= "4" //DevoluÁ„o de Mercadoria
   		End
	...
	Case (!Empty(aNfVinc) .And. aNota[5]$"B" .And. SF4->F4_AJUSTE <> "S")
		cTPNota:= "4" //DevoluÁ„o de Mercadoria, com referencia de nf origem
    ...
    ////
   	...
   	/*Ajuste para emitir notas do tipo devoluÁ„o Tag< finnfe> =4  sem necessidade de referenciar a nota original
    para os  CFOP  1.201, 1.202, 1.410, 1.411, 5,921 e 6,921 . Evitando a rejeiÁ„o 321- RejeiÁ„o: NF-e de devoluÁ„o de mercadoria n„o possui
...
If cVeramb >= "3.10"
	If !Empty(aDest[20]) .and. aDest[20] == "F"
		cString += '<indFinal>1</indFinal>' //1-OperaÁ„o com consumidor final
		cIndFinal:= "1"
	Else
	...
	If Empty(cIndPres)
		If aNota[5] == "N"
			cIndPres := "9" //OperaÁ„o n„o presencial
		Else
            cIndPres := "0" //0-N„o se Aplica
		End
	EndIf
	cString += '<indPres>'+cIndPres+'</indPres>' // PresenÁ„o do comprador no momento da OperaÁ„o
EndIf
cString += '</ide>'
Return( cString )
...
Static Function NfeDest(aDest,cVerAmb,aTransp,aCST,lBrinde)
	...
	If cVerAmb >= "3.1
		...
		ElSE
				...
				cIndicador := "9" //9-N„o Contribuinte: a IE do destinat·rio pode ser informada ou n„o, j· que algumas UF concedem inscriÁ„o estadual para n„o contribuintes.
				//No caso de operaÁ„o com o Exterior informar indIEDest=9 e n„o informar a tag IE do destinat·rio;
		...
		/*	indIEDest - Indicador da IE do destinat·rio
			...
			2=Contribuinte isento de InscriÁ„o no cadastro de Contribuintes do ICMS;
			9=N„o Contribuinte, que pode ou n„o possuir InscriÁ„o Estadual no Cadastro de Contribuintes do ICMS;
		*/
	Else
	...
	//Tratamento para atender Manual de OrientaÁ„o do Contribuinte vers„o 5.00 onde È ObrigatÛrio, nas operaÁıes que se beneficiam de incentivos fiscais existentes nas ·reas sob controle da SUFRAM
...
/*
    ...
    */
	...
	EndIf
		...
		/*ativar o parametro MV_DIDANFE" - Par‚metro para informar os dados da DI nas informaÁıes complementares do Xml/Danfe  */
...
/*Grupo de informaÁıes de exportaÁ„o para o item - vers„o 3.10*/
If cVerAmb >= "3.10" .and. Len(aExp)>0
	If lEECFAT
		/*Quando a terceira posiÁ„o do array estiver vazia ou possuir tamanho 0 È porque a informaÁ„o n„o existe no processo.
		  Quando n„o houver dados de retorno referente ao ato concessÛrio e a exportaÁ„o indireta, a posiÁ„o [3][3] ter· tamanho 0.
		  Quando houver ato concessÛrio, a informaÁ„o ser· retornada na posiÁ„o [3][3][1]. O tamanho dessa dimens„o corresponde ‡ quantidade de atos concessÛrios encontrados para o item.
		  Quando houver exportaÁ„o indireta, a informaÁ„o ser· retornada na posiÁ„o [3][3][2]. O tamanho dessa dimens„o corresponde ‡ quantidade de notas fiscais de remessa com fim especÌfico de exportaÁ„o encontrada para o item.
		*/
		If ConvType(aExp[03][1]) == "I50" .and. Len(aExp[03][3]) > 0
				...
				For nA:= 1 to Len(aExp[03][3][1])
					...
					//Caso n„o tenha I52, posiÁ„o 3 È retornada vazia
...
If Len(aComb) > 0  .And. !Empty(aComb[01])
	...
	Endif
	/*NT 2015/002
	379 - RejeiÁ„o: Grupo de Encerrante na NF-e (modelo 55) para CFOP diferente
	de venda de combustÌvel para consumidor final (CFOP=5.656, 5.667).
	*/
		...
		If Len(aIcms)>0
			...
			Else
				...
				If cVerAmb >= "3.10" .and. aCST[1] $ '51' .and. !Empty(aICMS[12]) .and. !lArt186
			        ...
			        /* Desabilitado conforme orientaÁıes do Forncedor TOTVS
					...
					*/					/*Na vers„o 3.10, para CST=51, O Valor do ICMS(vICMS) deve ser a diferenÁa do Valor do ICMS da OperaÁ„o (vICMSOp) e o Valor do ICMS diferido (vICMSDif),
					para n„o apresentar a rejeiÁ„o 353-Valor do ICMS no CST=51 n„o corresponde a diferenÁa do ICMS operaÁ„o e ICMS diferido*/


					//Comentado pela TOTVS cString += '<valor>'+ConvType(iIf(lIcmDevol,aICMS[07],0),15,2)+'</valor>'
					//Trecho fornecido pela TOTVS para a soluÁ„o do problema no PARANA
...
if !empty(aProd[15]) .And. !empty(cMotDesICMS) .or. ( !empty(cMotDesICMS).and. lIcmDevol .and. !Empty(nDesonICM) ) /*Conforme chamado TILXYR foi incluido o .OR. para incluir devoluÁ„o na mensagem*/
...
/*Nota TÈcnica 004 de 2011 conforme chamado - THCTB4 e conforme portaria n∫ 275/2009 do chamado TPIPVV */
...
/*1 ñ Microempresa Municipal; 2 ñ Estimativa; 3 ñ Sociedade de Profissionais;
4 ñ Cooperativa; 5 ñ Microempres·rio Individual (MEI);
6 ñ Microempres·rio e Empresa de Pequeno Porte (ME EPP)*/
Local nX     := 0
Local nBicm := 0
LOcal nVicm := 0
Local nBicmst := 0
...
//AlteraÁ„o para que o valor de PIS ST e COFINS ST venha a compor o valor da nota este valor se encontra na tag vOutros  (NT 2011/004). E devoluÁ„o de compra com IPI n„o tributado
...
If LjNfPafEcf(SM0->M0_CGC) .AND. SuperGetMV("MV_LJPAFEC",,.F.) .AND. !( FindFunction("LjNfNoPaf") .AND. LjNfNoPaf(SM0->M0_CGC) )
	...
	Else
		...
		Else	//Caso nao possua a function, traz como padrao dados da homologacao realizada em 2011 Laudo POL2002011
			cMd5Master	:= IIF(LJAnalisaLeg(57)[1],"EC28FFF525A8851AC2A3EAE572CCD26F",; 	//Vers„o especial Credenciada no RJ via Laudo POL2002011
													"B300947472D4C482B759DDE8BC07B828") 	//Vers„o padr„o laudo POL20020
...
If Len( aNfVinc ) > 0		//Nota de espÈcie NFE e CTE vinculada
	For nZ := 1  to Len( aNfVinc )
		...
		EndIf
	Next nZ
ElseIf Len( aRefECF ) > 0	 	//Nota de espÈcie ECF vinculada
	...
	For nX := 1  to Len( aRefECF )
		...
		EndIf
	Next Nx
ElseIf Len( aNfVincRur ) > 0 	//Nota de espÈcie NFP vincula
...
//Tratamento para atender o DECRETO N∫ 35.679, de 13 de Outubro de 2010 - Pernambuco para o Ramo de Auto PeÁas - TRGTM2
If lCustoEntr
	cString += "ICMS apurado nos termos do Decreto n∫ 35.679, de 13 de Outubro de 2010."
EndIf

//Tratamento para adcionar o valor do ICMS desonerado para informaÁ„o complementar da Danfe.
If nVicmsDeson >0
...
If Len(aExp)>0 .And. !Empty(aExp[01])
	If lEECFAT
	/*Se vers„o 2.00 considera o retorno das posiÁıes 1 e 2
		Se vers„o 3.10, considera array da posiÁ„o 4 do primeiro item
	*/
		If cVerAmb == "2.00"
			cString += '<exporta>'
...
@param	nPos 	PosiÁ„o ref. Aliq. Tributante: 30 - Aliquota Total
...
Verifica se nota de devoluÁ„o utiliza cliente de entrega da nota de origem.
