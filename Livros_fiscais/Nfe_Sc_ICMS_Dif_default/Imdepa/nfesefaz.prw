...
User Function XmlNfeSef(cTipo,cSerie,cNota,cClieFor,cLoja,cNotaOri,cSerieOri)
	...
	Local nValAux    	:= 0
	Local nValPisZF  	:= 0
	LOCAL NVALPIS_imd   := 0 // Obtem Valor do PIS nas Nf-e de Entrada Importacao, para imcluir o Valor nas Informacoes adicionais...
	LOCAL NVALCOF_imd   := 0 // Obtem Valor do PIS nas Nf-e de Entrada Importacao, para imcluir o Valor nas Informacoes adicionais...
	Local nValCofZF  	:= 0
	Local nPisRet   	:= 0
	Local nCofRet   	:= 0
	Local nInssRet  	:= 0
...
//////JULIO JACOVENKO, em 05/11/2014
	Local  cMarca_imd   := "" //| Obtem a Marca‹o da Capa do Processo para adicionar no Array de Volumes...
	Local  cNVolTr_imd  := "" //| Obtem a Numero de Volumes da Capa do Processo para adicionar no Array de Volumes...
	Local lNotaBenef	:= .F.
	Local lDifParc		:= .F.
...
//JULIO JACOVENKO, 07/07/2011 ////INCLUIDO PARA TRAMENTO DAS MENSAGENS NA DANFE

	Private cMCli_imd 	:= '' //| Criada para Armazenar as Mensagens Clientes Customizadas Imdepa...
	Private cLocEnt_imd	:= '' //| criada para Armazenar e Incluir os dados do Cliente Referente ao Local de Entrega...
	Private aMcod_imd	:= {} //| Obtem as Mensagens dos campos F4_FORMUL?? utilizadas na TES para incluir no Complemento da Nfe....
	Private LSU_Imd 	:=.T. //| Controle par a n‹o incluir mais de uma vez a mensagem referente ao Suframa ...

	Public lCondAd_Imd		  //| Controle para Apresentar a tela de Volumes apenas uma vez....

/////JULIO JACOVENKO, 28/11/2013 /////incluido para tratamento de transferencia entre filiais /////SuperGetMv("MV_IMDEPA")
	Private cImdepa     	:= GetMv('MV_IMDEPA')	//| Recebe o codigo de cliente Imdepa...
	Private cNumPedv_Imd   	:= space(0)				//| Obtem numero deo Pedido de Venda para incluir no Xml
	Private cNumPedc_Imd   	:= space(0)				//| Obtem numero deo Pedido de Compra para incluir no Xml

////JULIO JACOVENKO, 19/03/2014 ////incluido para tratamento de entrada importacao
	Private lImporta_Imd    := .F.	// Controle que identifica que se trata de uma nf de importacao... para tratamentos diversos...
	Private ForEST_Imd 	    := 'N'  // Identifica se Ž Importa‹o e Adiciona algumas Tags Obrigat—rias...

///////JULIO JACOVENKO, em 29/09/2014
///////quarda o valor do suframa

	Private nPIcmZF_Imd			:= 0    //| Armazena Aliquota ICMS quando for venda para Zona Franca, sera incluida nas mensagens da nota...
	...
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	   ...
	   //ALERT('...VAI CHAMAR A FUNCAO FATUPESO...')
		...
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		    ...
		    ///JULIO JACOVENKO, em 01/04/2014 //AJUSTADA ENDERECO //LOGRADOURO...LGR // NA VERSAO 3.10 ENDERECO,NUMERO E COMPLEMENTO E SEPARADO
				aadd(aDest,FisGetEnd(ALLTRIM(SA1->A1_TLOGEND)+' '+AllTrim(U_CorEnd(SA1->A1_END)),SA1->A1_EST)[1])

	          //JULIO JACOVENKO, em 01/04/2014 // INCLUIDO PARA AJUSTAR PARA PEGAR O NUMERO CORRETAMENTE...
				If SA1->A1_NUMEND > 0
            ...
            */
            ...
            ///COMPLEMENTO DE ENDERECO_COMPL  //JULIO JACOVENKO, em 02/04/2015  ///NA VERSAO 3.10 ENDERECO,NUMERO E COMPLEMENTO E SEPARADO

				aadd(aDest, FisGetEnd( ALLTRIM(SA1->A1_COMPEND),SA1->A1_EST)[4])
			...
			///JULIO JACOVENKO, em 05/09/2013 ///pegar email de tabela preparada para isto  ///ZAY010 	///ESTOU POSICIONADO NO SA1, entao procura no ZAY por
			///SA1->A1_FILIAL+SA1->A1_COD+SA1->A1_LOJA
			///////////////////////////////////////////////////////////////////////////////////////////////

				cEmailCli_Imd := U_xCLEARACENTOS( ALLTRIM(SA1->A1_MAILNFE) )

				aadd(aDest,cEmailCli_Imd)
			...
			//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
				If !SF2->F2_TIPO $ "DB"
					...
					If SF2->(FieldPos("F2_CLIENT"))<>0 .And. !Empty(SF2->F2_CLIENT+SF2->F2_LOJENT) .And. SF2->F2_CLIENT+SF2->F2_LOJENT<>SF2->F2_CLIENTE+SF2->F2_LOJA
						...
						aadd(aEntrega,ALLTRIM(SA1->A1_INSCR))
					EndIf
			    ...
			    ////JULIO JACOVENKO, em 15/04/2015
			    ////ajuste pois nao estava saindo o numero em alguns casos

					If ( SA1->A1_NUMEND > 0 )
						aadd(aDest,cValToChar(SA1->A1_NUMEND))
					Else
						aadd(aDest,"SN")
					EndIf
					...
					If !Upper(SA1->A1_EST) == "EX"
						...
						Else
							aadd(aDest,VldIE(ALLTRIM(SA1->A1_INSCR)))
						EndIF
					Else
						aadd(aDest,"")
					EndIf
			...
			///////////////////////////////////////////////////////////////////////////////////////////////
					...
					cEmailCli_Imd:=U_xCLEARACENTOS( ALLTRIM(SA1->A1_MAILNFE) )
			///cEmailCli_Imd:=U_xCLEARACENTOS(ALLTRIM(SA1->(Posicione("ZAY",1,XFILIAL('ZAY')+SA1->A1_COD+SA1->A1_LOJA,"ZAY_EMAIL"))) )
					aadd(aDest,cEmailCli_Imd)
				...
				While ( !Empty(cScan) )
					...
					If !Empty(cEspecie)
						...
						//aadd(aEspVol,{ cEspecie, FieldGet(FieldPos("F2_VOLUME"+cScan)) , SF2->F2_PLIQUI , SF2->F2_PBRUTO})
							aadd(aEspVol,{ cEspecie, FieldGet(FieldPos("F2_VOLUME"+cScan)) , SF2->F2_PLIQUI , SF2->F2_PBRUTO,cMarca_imd	,cNVolTr_imd})
						Else
			...
			//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
				...
				#IFDEF TOP
					...
					BeginSql Alias cAliasSD2
						...
						D2_ORIGLAN,D2_VALCF3,D2_VALIPI,D2_VALACRS,D2_VALICM,D2_PICM,D2_PDV %Exp:cField%
						FROM %Table:SD2% SD2
						WHERE
									...
									//para nota de entrada tipo devolucao o emitente eh o cliente ou o sigamat no caso de formulario proprio=sim
										If SD1->D1_TIPO$"DB"
											aadd(aNfVincRur,{SD1->D1_EMISSAO,SD1->D1_SERIE,SD1->D1_DOC,SF1->F1_ESPECIE,;
												...
												IIF(SD1->D1_FORMUL=="S",SM0->M0_INSC,ALLTRIM(SA1->A1_INSCR))})
...
// tratamenTo para os itens de transferencia entre filiais
				...
				// Localiza a planilha de transferencia
						...
						// atualiza a lista de pedidos de compra
											...
											If ! (SC7->C7_NUM $ cNumPedc_Imd)
												If .not. Empty(cNumPedc_Imd)
													cNumPedc_Imd := cNumPedc_Imd +'/ '+ SC7->C7_NUM
												Else
													cNumPedc_Imd := SC7->C7_NUM
												EndIf
											EndIf
							...
							If ! ((cAliasSD2)->D2_PEDIDO $ cNumPedv_Imd)
								If .not. Empty(cNumPedv_Imd)
									cNumPedv_Imd := cNumPedv_Imd + '/ ' + (cAliasSD2)->D2_PEDIDO
								Else
									cNumPedv_Imd := (cAliasSD2)->D2_PEDIDO
								EndIf
							EndIf
...
//////////////////////////////////////////////////////////////
			...
			///////JULIO JACOVENKO, 13/01/2014 /// Incluido o cusind na descricao do produto
			//+---------------------------------------------------------------------------------------------------------//
			//|ProjeTo Logico Nfe_preco de custo - TES 720 Vendas de Mercadorias sujeitas ao regime ST dentro do Estado
			//|Inserido por Edivaldo Gonsalves Cordeiro em 28/04/2010
			//+---------------------------------------------------------------------------------------------------------//
						...
						If (SA1->A1_CUSToNF='S' .AND. (cAliasSD2)->D2_FILIAL='05' .AND. (cAliasSD2)->D2_TES='720')
							nBaseIcms_Imd	:=	(cAliasSD2)->(U_nRetBaseRed( (cAliasSD2)->D2_FILIAL, (cAliasSD2)->D2_COD))
						EndIf
						...
						If (cAliasSD2)->D2_CF $"5655/6655" .OR. SB1->B1_GRTRIB = "001"
							nBaseIcmsTot_Imd:= nBaseIcmsTot_Imd + (cAliasSD2)->(U_nRetBaseRed( (cAliasSD2)->D2_FILIAL, (cAliasSD2)->D2_COD))
							nIcmsRetTot := nIcmsRetTot  + (cAliasSD2)->(U_nRetIcmsRet( (cAliasSD2)->D2_FILIAL, (cAliasSD2)->D2_COD))
						EndIf

		//	If nBaseIcms_Imd > 0
		//    	cCusto_Imd := " (* "+(AllTrim(STR(nBaseIcms_Imd)))+")"
		//    Endif

						If nBaseIcms_Imd > 0
							cCusto_Imd:= " (* BC= "+(AllTrim(STR(nBaseIcms_Imd)))+" IRET= "+(AllTrim(STR(nIcmsRet)))+")"
						EndIf


			//////////////////////////////////////////////////////////
			///JULIO JACOVENKO, em 14/01/2014
			///AJUSTE PARA MENSAGENS PADROES CHUMBADAS NO ///GXMLIMD, o certo era trabalhar com FORMULAS, mas 	///o tempo não permite agora.
			/////////////////////////////////////////////////////////////////////

						If !EMPTY(cCusto_Imd) .AND. SF2->F2_FILIAL = "09"
						...
						If !EMPTY(cCusto_Imd) .AND. SF2->F2_FILIAL = "13"
				   ...
				   //Base de calculo e valor do ICMS ST, cfe Art. 471 - Decreto 1.980/07 - RICMS/PR
						...
						If !EMPTY(cCusto_Imd) .AND. SF2->F2_FILIAL = "10"
							...
							ENDIF
						EndIf
						If  nBaseIcmsTot_Imd > 0 .AND. nIcmsRetTot > 0
							IF !AllTrim(" Base de calculo do ICMS Retido = "+ str(nBaseIcmsTot_Imd,12,2) + " e ICMS Retido = "+ str(nIcmsRetTot,12,2) +" , conforme Convenio 03/99 ") $ cMensFis
								cMensFis += " Base de calculo do ICMS Retido = "+ str(nBaseIcmsTot_Imd,12,2) + " e ICMS Retido = "+ str(nIcmsRetTot,12,2) +" , conforme Convenio 03/99 "
							ENDIF
						EndIf
            ...
            ///////////////////////////////////////////////////////////////////
             ...
             ///
						...
						cDescProd:=ALLTRIM((cAliasSD2)->(POSICIONE("SBM",1,XFILIAL("SMB")+SB1->B1_GRUPO,"BM_DESC")))+" "+SB1->B1_DESC+cCusto_Imd
			//cDescProd := IIF(Empty(SC6->C6_DESCRI),SB1->B1_DESC,SC6->C6_DESCRI)
				...
				/////ajustado para cCodCli_Imd substituir D2_COD
                ////TRATANDO CODIGO E DESCRICAO
                //CDESCRI
				//		cDesprod:=ALLTRIM((cAliasSD2)->(POSICIONE("SBM",1,XFILIAL("SMB")+SB1->B1_GRUPO,"BM_DESC")))+" "+SB1->B1_DESC+cCusto_Imd
						If SA7->(DbSeek(XFILIAL("SA7")+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD,.F.)) .and. !EMPTY(SA7->A7_CODCLI)
							cCodCli_Imd := AllTrim(SA7->A7_CODCLI)+' '       // Codigo do produTo do cliente
						Else
							cCodCli_Imd := ' '  //(cAliasSD2)->D2_COD
						EndIf
				//para o SD2
						cDescProd:=AllTrim(cCodCli_Imd)+cDescProd
                ////aqui, busca a mensagem fiscal conforme a TES
                ////
						If !(cAliasSD2)->D2_TIPO $"B/D" .AND. !Empty(SA1->A1_SUFRAMA) .AND. LSU_Imd // Mensagem Suframa
							LSU_Imd := .F.
							cMCli_imd += " |Inscr. SUFRAMA:"+ AllTrim(SA1->A1_SUFRAMA) + Space(01) //FConvChar(AllTrim(SA1->A1_SUFRAMA)) + Space(01)

								/*
								If nDescSuf > 0
									cMCli_imd += ' DesconTo Sefaz '+ AllTrim(Str(nPicm)) +'% R$ '+ TransForm(nDescSuf,'@E 999,999.99')
								EndIf
								*/
                ...
                //IIF(!(cAliasSD2)->D2_TIPO$"IP",(cAliasSD2)->D2_TOTAL+nDesconto+(cAliasSD2)->D2_DESCZFR,IIF(((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. SubStr(SM0->M0_CODMUN,1,2) == "31") .Or. ((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. "RESSARCIMENTO" $ Upper(cNatOper) .And. "RESSARCIMENTO" $ Upper(cDescProd)),(cAliasSD2)->D2_TOTAL,0)),;
						...
						SB1->B1_EX_NCM,;
							...
							SB1->B1_UM,;
							(cAliasSD2)->D2_QUANT,;
							IIF(!(cAliasSD2)->D2_TIPO$"IP",(cAliasSD2)->D2_TOTAL+nDesconto,IIF(((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. SubStr(SM0->M0_CODMUN,1,2) == "31") .Or. ((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. "RESSARCIMENTO" $ Upper(cNatOper) .And. "RESSARCIMENTO" $ Upper(cDescProd)),(cAliasSD2)->D2_TOTAL,0)),;
							...
							(cAliasSD2)->D2_VALFRE,;
							(cAliasSD2)->D2_SEGURO,;
							(nDesconto+nDescIcm+nDescRed),;   //prod[15]
						...
						IIf(SubStr(SM0->M0_CODMUN,1,2) == "35" .And. cTpPessoa == "EP" .And. nDescIcm > 0, nDescIcm,0),;
							IIF((cAliasSD2)->(FieldPos("D2_TOTIMP"))<>0,(cAliasSD2)->D2_TOTIMP,0),;   //Total imposto carga tributária.
                ...
                //aadd(aPedCom,{})
					...
					If AliasIndic("CD6")  .And. CD6->(FieldPos("CD6_QTAMB")) > 0 .And. CD6->(FieldPos("CD6_UFCONS")) > 0  .And. CD6->(FieldPos("CD6_BCCIDE")) > 0 .And. CD6->(FieldPos("CD6_VALIQ")) > 0 .And. CD6->(FieldPos("CD6_VCIDE")) > 0
						aadd(aComb,{ALLTRIM(GETMV("MV_ANPGRAX")),CD6->CD6_SEFAZ,CD6->CD6_QTAMB,CD6->CD6_UFCONS,CD6->CD6_BCCIDE,CD6->CD6_VALIQ, CD6->CD6_VCIDE, IIf(CD6->(FieldPos("CD6_MIXGN")) > 0,CD6->CD6_MIXGN,"") })
				    Elseif AliasIndic("CD6")  .And. CD6->(FieldPos("CD6_QTAMB")) > 0 .And. CD6->(FieldPos("CD6_UFCONS")) > 0
				    	aadd(aComb,{ALLTRIM(GETMV("MV_ANPGRAX")),CD6->CD6_SEFAZ,CD6->CD6_QTAMB,CD6->CD6_UFCONS})
					Else
						aadd(aComb,{})
					EndIf
					*/
					...
					//Anfavea Cabecalho
					   ...
					   //PEGA OS VALORES E PERCENTUAL DO INNCENTIVO NOS ITENS NA SFT.
						...
						// Verifica se existe percentual de reducao na SFT referête ao RICMS 43080/2002 MG.
							...
							Do Case
						//CASE , condição fornecida pela TOTVS
					...
					//Tratamento para que o valor de PIS ST e COFINS ST venha a compor o valor total da tag vOutros  (NT 2011/004). E devolução de compra com IPI não tributado
						...
						If (cAliasSD2)->D2_TIPO == "I"
							...
							Endif
						Else
							aTotal[02] += ((cAliasSD2)->D2_VALBRUT) //- (cAliasSD2)->D2_DESCZFR)
						EndIf
					...
					If MsSeek(xFilial("SF3")+SF2->F2_CLIENTE+SF2->F2_LOJA+SF2->F2_DOC+SF2->F2_SERIE)
						If !SF3->F3_DESCZFR == 0 .or. ( lInfAdZF .and. nValBse > 0 )
							...
							//cMensFis += "Descontos Ref. a Zona Franca de Manaus / ALC. ICMS - R$ "+str(nValBse-SF2->F2_DESCONT-nValPisZF-nValCofZF,13,2)+", PIS - R$ "+ str(nValPisZF,13,2) +"e COFINS - R$ " +str(nValCofZF,13,2)
								cMensFis += "Descs. Ref. a Zona Franca de Manaus/ALC. Desc Sefaz:"+str(nPIcmZF_Imd,5,2)+" % ICMS - R$ "+str(SF3->F3_VALOBSE,13,2)+", PIS - R$ "+ str(nValPisZF,13,2) +"e COFINS - R$ " +str(nValCofZF,13,2)
							...
							//cMensFis += "Desconto Ref. ao ICMS - Zona Franca de Manaus / ALC. R$ "+str(nValBse-SF2->F2_DESCONT-nValPisZF-nValCofZF,13,2)
								cMensFis += "Desc. Ref. ao ICMS - Zona Franca de Manaus/ALC. Desc Sefaz:"+str(nPIcmZF_Imd,5,2)+" % R$ "+str(SF3->F3_VALOBSE,13,2)
							Else
					    	//cMensFis += "Total do desconto Ref. a Zona Franca de Manaus / ALC. R$ "+str(nValBse-SF2->F2_DESCONT,13,2)
								cMensFis += "Total do desconto Ref. a Zona Franca de Manaus/ALC. Desc Sefaz:"+str(nPIcmZF_Imd,5,2)+" % R$ "+str(nValBse-SF2->F2_DESCONT,13,2)
							EndIF
						EndIf
					EndIf
				EndIF
		    ...
		    //Mensgaem para ICMS Particionado - Convênio ICMS Nº 51/00,
				...
				If ((SubStr(SM0->M0_CODMUN,1,2) == "35" .Or. SubStr(SM0->M0_CODMUN,1,2) == "29") .and. "REMESSA POR CONTA E ORDEM DE TERCEIROS" $ Upper(cNatOper) .and. lOrgaoPub )
	...
	Else
		...
		// Vou recuperar as inFormacoes para quando For nota de importacao
			...
			//cComplNf:= __cNForig //COMPLEMENTo DA NF
				...
				cMarca_imd := ALLTRIM(SW6->W6_MARCA)
				cNVolTr_imd := ALLTRIM(SW6->W6_NUMERAC)




				nII_Imd     := SF1->F1_II      //IMPOSTo DE IMPORTACAO

				nSG_Imd      := SF1->F1_CIf - SF1->F1_FOB_R - nFR //SEGURO
		...
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
			...
			//cEmailCli_Imd:=U_xCLEARACENTOS(ALLTRIM(SA1->(Posicione("ZAY",1,XFILIAL('ZAY')+SA1->A1_COD+SA1->A1_LOJA,"ZAY_EMAIL"))) )
				...
				//aadd(aDest,IIF(Empty(SA2->A2_PAIS),"1058"  ,Posicione("SYA",1,xFilial("SYA")+SA2->A2_PAIS,"YA_SISEXP")))
					...
					aadd(aDest,"")//Posição 20
					aadd(aDest,SA2->A2_PFISICA)//21-Identificação estrangeiro
///////////////////TRATAMENTO ADEST////SE DER PAU
///////////////////PEGAR DA PRODUCAO
///////////////////LINHA 4970 - JULIO REF 16/05/2014
					...
					While ( !Empty(cScan) )
						...
						If !Empty(cEspecie)
						    ...
						    ///JULIO JACOVENKO, em 18/11/2014
								...
								aadd(aEspVol,{ cEspecie, FieldGet(FieldPos("F1_VOLUME"+cScan)) , SF1->F1_PLIQUI , SF1->F1_PBRUTO,cMarca_imd,cNVolTr_imd})
			...
			//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
				...
				#IFDEF TOP
					...
					BeginSql Alias cAliasSD1
						...
						D1_NUMLOTE,D1_CUSTO,D1_ORIGLAN,D1_DESCICM,D1_CIF,D1_II,D1_FORMUL,D1_VALPS3,D1_ORIGLAN,D1_VALCF3,D1_TESACLA  %Exp:cField%
						FROM %Table:SD1% SD1
						WHERE
				...
				//aAdd(aInfoItem,{(cAliasSD1)->D1_PEDIDO,(cAliasSD1)->D1_ITEMPC,AllTrim(If(lImporta_Imd,(cAliasSD1)->D1_TESIMP,(cAliasSD1)->D1_TES)),(cAliasSD1)->D1_ITEM})
               ...
               ///////////////////////////////////////////////////////////////TRATAMENTO DE CODIGO E DESCRICA DE PRODUTO
				...
				///tirar o cCusto_Imd, poi he nota de entrada....
				///
				//////////////////////////////////////////////////////////////////////////////////////////////////////////



					cDescProd := ALLTRIM((cAliasSD1)->(POSICIONE("SBM",1,XFILIAL("SMB")+SB1->B1_GRUPO,"BM_DESC")))+" "+SB1->B1_DESC //+cCusto_Imd
...
*/
        ...
        ///compatibilizar com UNI5
					...
					IF lImporta_Imd
						NVALCOF_imd += (cAliasSD1)->D1_VALIMP5
						NVALPIS_imd += (cAliasSD1)->D1_VALIMP6
					ENDIF
        ////////////////////////////////////////////////////
...
////tratamento transferencia
/////////////////////////////////////////////////////////////
	If ! EMPTY(cNumPedc_Imd)
		cNumPedc_Imd := " Pedido de Compra: "+cNumPedc_Imd
	EndIf

	If ! EMPTY(cNumPedv_Imd)
		cNumPedv_Imd := " Pedido de Venda: "+cNumPedv_Imd
	EndIf
...
///////////////////
    			...
    			/////////////////////ISTO E FEIO NO FINAL DA ROTINA COM A VARIAVEL cMCli_imd
	...
	IF !lImporta_Imd
		...
		cMCli_imd+="  Em caso de VIOLACAO da FITA LACRE, conferir no Recebimento. "
                //cMCli_imd+="Pedido de Venda: "+SC5->C5_NUM+" "

                ///JULIO JACOVENKO, em 29/11/2013
                ///tratamento transferencia
		cMCli_imd +=  cNumPedc_Imd + cNumPedv_Imd + " "
		...
		If !empty(AllTrim(SC5->C5_OCCLI))
					...
					//cMCli_imd += ' Ord.Comp: '+AllTRIM(cOrdComp)       //AllTrim(SC5->C5_OCCLI)
			cMCli_imd += ' Ord.Comp: '+AllTrim(SC5->C5_OCCLI)
		EndIf
		...
		If SF2->F2_CLIENTE = "N00000"
			cFormul := U_FRetForm()
			If !EMPTY(cFormul)
				cMCli_imd += ' '+&(POSICIONE("SM4",1,XFILIAL("SM4")+cFormul,"M4_ForMULA"))
			EndIf
		EndIf
		...
		IF ALLTRIM(CNOMEVEND)<>''
			cMCli_imd+="  Vendedor: "+ALLTRIM(cNOMEVEND)
		ELSE
			cMCli_imd+=' '
		ENDIF
		...
		If !empty(AllTrim(SA1->A1_TLOGENT))
			cLocEnt_imd += ' '+AllTrim(SA1->A1_TLOGENT)
		EndIf
		If !Empty(AllTrim(SA1->A1_ENDENT))
			cLocEnt_imd += ' '+AllTrim(SA1->A1_ENDENT)+','
		EndIf
		If !Empty(AllTrim(Str(SA1->A1_NUMENTR)))
			cLocEnt_imd += ' N.'+AllTrim(Str(SA1->A1_NUMENTR))
		EndIf
		If  !Empty(AllTrim(SA1->A1_COMPLen))
			cLocEnt_imd += ' '+AllTrim(SA1->A1_COMPLen)
		EndIf
		If  !Empty(AllTrim(SA1->A1_BAIRROE))
			cLocEnt_imd += ' '+AllTrim(SA1->A1_BAIRROE)
		EndIf
		If  !Empty(AllTrim(SA1->A1_MUNE))
			cLocEnt_imd += ' '+AllTrim(SA1->A1_MUNE)
		EndIf
		If  !Empty(AllTrim(SA1->A1_ESTE))
			cLocEnt_imd += ' '+AllTrim(SA1->A1_ESTE)
		EndIf
		If  !Empty(AllTrim(SA1->A1_CEPE))
			cLocEnt_imd += ' '+AllTrim(SA1->A1_CEPE)
		EndIf

		IF SF2->F2_TIPO <> "B"
			If ALLTRIM(cLocEnt_imd)<>''
				cLOCALENT:=cLocEnt_imd
				cMCli_imd+="  Local de Entrega: "+cLOCALENT+" "
			else
				cMCli_imd:=' '
			endif
		ENDIF
		...
		If ! Empty(SC5->C5_CLIENTR)
			...
			cMCli_imd += ' Nome: '+SA1->A1_NOME
			cMCli_imd += ' Endereco: '+_cEnd
			cMCli_imd += ' Inscricao Estadual: '+ALLTRIM(SA1->A1_INSCR)
			cMCli_imd += ' CNPJ: '+SA1->A1_CGC
			lNFordem := .F.
		...
		If ! Empty(SC5->C5_NFVORD)
			...
			cMCli_imd += ' Nota Fiscal '+SC5->C5_NFVORD+'   Data: '+DToc(SF2->F2_EMISSAO)
			cMCli_imd += ' Nome: '+SA1->A1_NOME
			cMCli_imd += ' Endereco: '+_cEnd
			cMCli_imd += ' Inscricao Estadual: '+ALLTRIM(SA1->A1_INSCR)
			cMCli_imd += ' CNPJ: '+SA1->A1_CGC
	...
	ELSE
		...
		cMCli_imd+="  Codigo IMDEPA do Fornecedor: "+cCDOIND+" "
	ENDIF
/////JULIO JACOVENKO, em 19/03/2014
/////quando nf entrada importacao
	...
	If lImporta_Imd
		If !EMPTY(cNumDI)
			cMCli_imd += "**NUMERO DA DI " + cNumDI
		EndIf

		If !EMPTY(cNumProc)
			cMCli_imd += " NUMERO DO PROCESSO " + cNumProc
		EndIf

		/*
		If SF1->F1_TIPO <> "N" .and. !Empty(__cNForig)
			cMCli_imd += " COMPLEMENTo DA NF " + __cNForig
		EndIf
		*/

		If !EMPTY(SF1->F1_DESPESA)
			cMCli_imd += " DESPESA ADUANEIRA: " + STR(SF1->F1_DESPESA,12,2)
		EndIf

		/*
		If !EMPTY(nSG_Imd)
			cMCli_imd += " SEGURO: " + STR(nSG_Imd,12,2)
		EndIf
		*/

		If !EMPTY(nFR)
			cMCli_imd += " FRETE: " + STR(nFR,12,2)
		EndIf

		If !EMPTY(nII_Imd)
			cMCli_imd += " IMPOSTO DE IMPORTACAO: " + STR(nII_Imd,12,2)
		EndIf


		If !EMPTY(nValPis)
			cMCli_imd += " PIS: " + STR(nValPis,12,2)
		EndIf

		If !EMPTY(nValPis)
			cMCli_imd += " COFINS: " + STR(nValCof,12,2)
		EndIf
	EndIf

	IF ! EMPTY(SF2->F2_VEICUL2)

		cMCli_imd += " Placa do Reboque: " + SF2->F2_VEICUL2
...
//Geracao do arquivo XML
	If !Empty(aNota)
		cString := ""
		cString += NfeIde(@cNFe,aNota,cNatOper,aDupl,aNfVinc,cVerAmb,aNfVincRur,aRefECF,cIndPres,aDest,aProd)
...
////aqui entra a geracao do boleto (logo apos a geração do XML)
////como he hoje.
////
...
Static Function NfeIde(cChave,aNota,cNatOper,aDupl,aNfVinc,cVerAmb,aNfVincRur,aRefECF,cIndPres,aDest,aProd)
...
///
	...
	Case ( ( AllTrim( SF4->F4_CF ) $ cMVCfopTran ) .and. ( SF4->F4_SITTRIB == "90" ) .and.  ( SF4->F4_AJUSTE == "S" ) )
		cTPNota:= "3"
	Case (cVeramb >= "3.10" .and. (!Empty(aNfVinc) .Or. !Empty(aRefECF))) .and. (aNota[5] $ "D" .And. SF4->F4_AJUSTE <> "S")
...
Static Function NfeDest(aDest,cVerAmb,aTransp,aCST,lBrinde)
	...
	If cVerAmb >= "3.10"
		...
		If ConvType(aDest[18]) <> "2" .and. !Empty(aDest[14])
		...
		Else
			...
			Else
				...
				If ConvType(aDest[17]) <> "2" .And. !"EX" $ aDest[08]
					cString += '<IE>'+ConvType(iIf(!lBrinde,aDest[14],SM0->M0_INSC))+'</IE>'
				EndIf
			EndIf
		EndIf
...
//Tag <vDesc>
	...
	//Quando eh Zona Franca de Manaus
		If Len(aICMSZFM) > 0 .And. Len(aCST) > 0
			If !Empty(aICMSZFM[1]) .And. (aCST[1] $ '40,41,50')
			///erro 610 quando desconto ZF
			//cString += NfeTag('<vDesc>' ,ConvType((aProd[31]+aProd[32]),15,2))
			//JULIO JACOVENKO, em 07/04/2015
			//	(cAliasSD2)->D2_DESCZFP,;			//Desconto Zona Franca PIS     aprod[31]
			//	(cAliasSD2)->D2_DESCZFC,;			//Desconto Zona Franca CONFINS aprod[32]
...
//Ver II - Average - Tag da Declaração de Importação aDI
	...
	*/
	...
	If cVerAmb >= "3.10" .and. Len(aExp)>0
		If lEECFAT
			If ConvType(aExp[03][1]) == "I51" .And. !Empty(aExp[03][3])
				cString += '<detExport>'
				cString += '<Draw>'+ConvType(aExp[03][03],11)+ '</Draw>'
				If !Empty(aExp[04][03])
					cString += '<exportInd>'
					cString += '<nre>'+ConvType(aExp[04][03],12)+ '</nre>'
					cString += '<chnfe>'+ConvType(aExp[05][03],44)+ '</chnfe>'
					cString += '<qExport>'+ConvType(aExp[06][03],15,4)+ '</qExport>'
					cString += '</exportInd>'
				EndIf
				cString += '</detExport>'
			EndIf
		Else
			For nX := 1 To Len(aExp)
				If ConvType(aExp[1][03][1]) == "I51" .And. !Empty(aExp[nX][03][3])
					cString += '<detExport>'
	...
	If  !lIssQn
		If cMVCODREG == "1" .and. SF4->(FieldPos("F4_CSOSN"))>0 .And. !Empty(SF4->F4_CSOSN)
			...
			If cCsosn$"900" .And. Len(aIcms)>0
				...
				Else
					If cVeramb >= "3.10"
						cString += '<pRedBC>'+ConvType(aICMS[04],7,4)+'</pRedBC>'
					Else
			...
			If !( aNfVinc[nZ][2] + aNfVinc[nZ][3] ) $ cChvNFe
				...
				if SM0->M0_ESTENT == 'SP'  // Conforme chamado TIGZGX feito com a Consultoria tributaria
					If Len (aNfVinc[nZ] ) >= 8 .and.  !Empty(aNfVinc[nZ][08])
						cString += "Valor da Operacao do Documento de Origem: " + ConvType(aNfVinc[nZ][08],15,2) +"."
					endif
				endif
			EndIf
		Next nZ
...
If Len(aExp)>0 .And. !Empty(aExp[01])
	If lEECFAT
		...
		If cVerAmb >= "3.10"
			cString += NfeTag('<locdespacho>' ,ConvType(aExp[01][07][03]))
		EndIf
		cString += '</exporta>'
	Else
		cString += '<exporta>'
...
**************************************************************************
	...
	For i := 1 To 3
		...
		If !Empty(&cCampo.)
			If ascan(aMcod_imd, (&cCampo.)) > 0
				Return('')
			Else
				aadd(aMcod_imd,	(&cCampo.))
			EndIf
...
**********************************************************************
	Local cAcentos  := "çÇ€‡áéíóúÁÉÍÓÚâêîôûÂÊÎÔÛàéíóúÁÉÍÓÚäëïöüÄËÏÖÜãõÃÕŽÀÅ…† „¦åÈˆ‚èÌ¡ÒÖ“”•¢§ðÙ£ùÑñ%$þ÷Ý°ºª¬ƒ&"
...
**************************************************************************
	Local cAcenTos  	:= "çÇ€‡áéíóúÁÉÍÓÚâêîôûÂÊÎÔÛàéíóúÁÉÍÓÚäëïöüÄËÏÖÜãõÃÕŽÀÅ…† „¦åÈˆ‚èÌ¡ÒÖ“”•¢§ðÙ£ùÑñþ÷Ý°ºª¬ƒ&'"
...
Static Function PercTrib(aProd , lProdItem)
	...
	If lProdItem
		...
		DbSelectArea("SB1")
		DbSetOrder(1) // B1_FILIAL+B1_COD
		DbSeek(xFilial("SB1")+allTrim(aProd[2]))
	...
	Else
		cPercTrib	:= ConvType((nTotalCrg / nTotNota) * 100,15,2)
	EndIf
