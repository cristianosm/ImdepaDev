...
//| Objetos
	...
	Private lMostra		:= .F.
	Private FilComEst02 := GetMv("IM_OPERE02")
...
*******************************************************************************
	...
	U_ExecMySql(cSql,"cSQL","Q",lMostra)
...
//| Query Busca Produtos
	...
	U_ExecMySql(cQuery,"cQuery","Q",lMostra)
...
//| Abri e Ordena Tabelas
	...
	// Tratamento para identificar as filiais que possuem estoque
		...
		Else
			cQuery += "WHERE B2_FILIAL IN ("+cFilEst+") AND B2_COD = '"+cQuery->B1_COD+"' AND   B2_LOCAL = '01' AND   D_E_L_E_T_ = ' '"
		EndIf
		cQuery +=" Group by b2_filial "
		cQuery +=" ORDER BY B2_FILIAL "

		U_ExecMySql(cQuery, "VEST", "Q" ,lMostra)
	...
	//| Produtos com Equivalente / Similar
		...
		U_ExecMySql(cQuery,"cQueryPla","Q",lMostra)
	...
	//³conforme solicitação da Diretoria
        ...
        // Verifica se tem Acréscimo MVA ou Desc/Acres especifico
	  If lTemAcreMva
           ...
           If (SF4->(DbSeek(xFilial("SF4")+cPrevTes,.F.)))
               If ((SA1->A1_EST==cEstATU .AND. cEstATU=='SP' .AND.  Alltrim(SF4->F4_CF) == "5405") .OR.;
                   (SA1->A1_EST==cEstATU .AND. cEstATU=='PR' .AND. cPrevTes=='723' .AND. (Alltrim(SB1->B1_ORIGER) <> "F/NCZADO" .OR. Alltrim(SB1->B1_ORIGER) <> "IMPORTADO"  )) .OR.;
                   (SA1->A1_EST==cEstATU .AND. cEstATU=='RS' .AND. cPrevTes=='721' .AND. (Alltrim(SB1->B1_ORIGER) <> "F/NCZADO" .OR. Alltrim(SB1->B1_ORIGER) <> "IMPORTADO"  )))
	  ...
	  Endif
		...
		//Acrescimo MVA
	 	  If nAcreMva <> 0
	 	    nPrecoPro := A410Arred(nPrecoPro +((nPrecoPro*nAcreMva)/100))
	 	  Endif
	 	 */

	 	 	//Acrescimo MVA ou Desconto Especifico da Filial corrente
	 	   If nAcreMva <> 0
...
*/
	...
	cQuery += " SELECT "
	cQuery += "    ZH_PROD, ZH.ZH_EQUI, B1.B1_COD, B1.B1_DESC, B1_REFER, B1.B1_MARCA, B1_SGRB1 "
	cQuery += " FROM "
	cQuery += "    SZH010 ZH, SB1010 B1, ( "
	cQuery += "                            SELECT B2_FILIAL, B2_COD, SUM(B2_QATU) B2_QATU, SUM(B2_RESERVA) B2_RESERVA, SUM(B2_QACLASS) B2_QACLASS, SUM(B2_QPEDVEN) B2_QPEDVEN "
	cQuery += "                            FROM SB2010 "
	...
	cQuery += "    AND ZH.ZH_FILIAL = '" + xFilial( "SZH" ) + "' "
	cQuery += "    AND ZH.ZH_PROD  = '" +  cProd + "' "
	cQuery += "    AND ZH.ZH_PROMBOK = 'S' "
	cQuery += "    AND ZH.ZH_EQUI = B1.B1_COD "

	cQuery += "    AND B1.B1_FILIAL = '" + xFilial( "SB1" ) + "' "
	//cQuery += "    AND B1.B1_LOCPAD = '01'
	cQuery += "    AND B1.B1_EQUIVAL = 'S' "
	cQuery += "    AND B1.D_E_L_E_T_ = ' ' "

	cQuery += "    AND B2.B2_FILIAL = B1.B1_FILIAL "
	cQuery += "    AND B2.B2_COD = ZH.ZH_EQUI "
	cQuery += "    AND  ( B2.B2_QATU - B2.B2_RESERVA - B2.B2_QACLASS - B2.B2_QPEDVEN ) > 0 "

	cQuery += " ORDER BY "
	cQuery += "    ZH.ZH_PROD "

	U_ExecMySql(cQuery, "QRY_EQUI", "Q", lMostra )
