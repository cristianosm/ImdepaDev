...
/*
@ 010,273 MsGet oGrp VAR cGrp Picture X3Picture('BM_GRUPO') Size 050,010 of oFolder:aDialogs[1] PIXEL //F3 "SBM"
@ 025,273 MsGet oGr1 VAR cGr1 Picture X3Picture('ZA_COD')	Size 050,010 of oFolder:aDialogs[1] PIXEL //F3 "SZA"
@ 040,273 MsGet oGr2 VAR cGr2 Picture X3Picture('ZB_COD')	Size 050,010 of oFolder:aDialogs[1] PIXEL //F3 "SZB"
@ 055,273 MsGet oGr3 VAR cGr3 Picture X3Picture('ZC_COD')	Size 050,010 of oFolder:aDialogs[1] PIXEL //F3 "SZC"
*/
...
//| Desenha o Cabecalho das Filiais
	...
	nXLinha += 5;@ nXLinha,004  SAY Space(84)+' '+' '+' '+ ' ' + c1Cabecalho +'P' PIXEL FONT oFONT
	nXLinha += 5;@ nXLinha,004  SAY Space(84)+' '+' '+' '+ ' ' + c2Cabecalho +'L' PIXEL FONT oFONT

/*

oBtn_estoqueFils := TButton():New(nXLinha,270,"",oDlg01,{|| fTeste(@oListPRO,oListPRO:nAt,@aListPRO)  },;
18,14,,,.F.,.T.,.F.,'Visualizar Estoque das outras filiais',.F.,,,.F. )

oBtn_estoqueFils:SetCss("QPushButton{ background-image: url(rpo:watch_mdi.png);"+;
...
//oTBitmap := TBitmap():Create(oDlg01,nXLinha,270,260,184,,'watch_mdi.png',.T.,            {||Alert("Clique em TBitmap1")},,.F.,.F.,,,.F.,,.T.,,.F.)
//oTBitmap := TBitmap():Create(oDlg01,nXLinha,146,260,184,,'xclose.png',.T.,            {||Alert("Clique em TBitmap1")},,.F.,.F.,,,.F.,,.T.,,.F.)

	@ nXLinha,004  SAY " "+PadR("ESTOQUE",09)+"  " PIXEL FONT oFONT
	nXLinha += 5;@ nXLinha,004  SAY " "+PadR("DISP.",09)+"  "+PadR("REFERENCIA",20)+"  "+PadR("MARCA",20)+"  "+PadR("R$ PRECO",12)+"  "+PadR("CODIGO",14)+'E'+' '+'S'+ ' ' + c3Cabecalho +'A' PIXEL FONT oFONT

	nXLinha += 102;TBtnBmp2():New(nXLinha,542,030,030,'watch_mdi.png',,,,{|| ToolTipEst(@oListPRO,oListPRO:nAt,@aListPRO) },oDlg01,"Estoque das outras Filiais",{||},,.T. )
...
//| Fim  Desenho
	...
	oBtn_localiza := TButton():New(118,453,"",oDlg01,{|| fLocaliza1(),oListPRO:cToolTip:='' },;
		39,18,,,.F.,.T.,.F.,'Localizar',.F.,,,.F. )

	oBtn_localiza:SetCss("QPushButton{ background-image: url(rpo:localiza_mdi.png);"+;
		" background-repeat: none; margin: 2px }")

//@ 120,455 Button OemToAnsi("Localizar") 	Size 036,015 of oDlg01 PIXEL Action fLocaliza1()



	oBtn_avancado := TButton():New(134,453,"",oDlg01,{|| (IniTela(), oDlg01:End()) },;
		39,18,,,.F.,.T.,.F.,'Avançado',.F.,,,.F. )

	oBtn_avancado:SetCss("QPushButton{ background-image: url(rpo:sduseek.png);"+;
		" background-repeat: none; margin: 2px }")
//@ 135,455 Button OemToAnsi("Avancado ")		Size 036,015 of oDlg01 PIXEL Action (IniTela(), oDlg01:End())


	oBtn_sair     := TButton():New(150,453,"",oDlg01,{|| (lSair := .F.,oDlg01:End()) },;
		39,18,,,.F.,.T.,.F.,'Sair',.F.,,,.F. )

	oBtn_sair:SetCss("QPushButton{ background-image: url(rpo:xclose.png);"+;
		" background-repeat: none; margin: 2px }")
...
//@ 150,455 Button OemToAnsi("&Sair" ) 		Size 036,015 of oDlg01 PIXEL Action (lSair := .F.,oDlg01:End())
...
//M->UB_PRODUTO 	:= SubStr(aArray[nIt],71,13)
...
/* //Desativado as ofertas automáticas

//| Jorge Oliveira - 04/01/2011 - Chamado AAZTEJ - Colocar na tela o Produto similar da Marca FAG
If FUNNAME() <> "IMDB2BPV"
	...
	If AllTrim( SB1->B1_MARCA ) == "INA" .And. !Empty( AllTrim( SB1->B1_SGRB1 ) )

		MsAguarde( {|| U_F190EQUI( SB1->B1_COD, SB1->B1_SGRB1 ) }, "Aguarde...", "Consultando Produto equivalente..." )

	EndIf

   // Oferta BOOK somente para orçamentos
	If SB1->B1_GRUPO=="0006"  .AND. M->UA_OPER=='2'
	  MsAguarde( {|| U_F190EBOOK( SB1->B1_COD ) }, "Aguarde...", "Consultando Produto equivalente Promo Book..." )
	Endif
...
//Limpa a linha atual do produto no ListBox , forçando o usuario a executar uma nova pesquisa em caso de desejar novamente o uso do produto em questão
//Somente limpa se a quantidade selecionada for maior que zero
//If nQtdAux>0
//	aArray[nIt]:=' '
//Endif


//aArray[nIt] := Str(nQtdAux,9,2)+Substr(aArray[nIt],10,200) // faz isso na validação

//aArray[nIt]  :=SubStr(aArray[nIt],56,12)='33.12'
//Msginfo(SubStr(aArray[nIt],56,12))
//aArray[nIt]  :=SubStr(aArray[nIt],56,12)
...
//Endif
	...
	//nQtdAux 	:= M->UB_QUANT // Jorge Oliveira - 21/02/2011 - Comentada essa linha
	ELSE
		nQtdAux := 0
		lValidouQTD := .T.
	ENDIF
...
//cMsgPostit:=U_ToolTipPrecProdCli(.F.)
//oListPRO:cTooltip:=cMsgPostit
...
//CQUERY += "                LEFT JOIN (  SELECT SB2.B2_COD,    MAX (SB2.B2_QATU) CASA "
	...
	CQUERY += "                              WHERE SB2.B2_FILIAL IN ('02','04','05','06','09','10','12','13') "
	...
	CQUERY += "                              AND SB2.B2_LOCAL   IN ('01','02') "
	...
	CQUERY += "  AND(SB2.B2_LOCAL   IN ('01','02') OR  SB2.B2_LOCAL IS NULL) "
...
CQUERY += "        SB1.B1_EQUIVAL, "
CQUERY += "        SB1.B1_SIMILAR, "
CQUERY += "        NVL(SB2.B2_QATU,0) QTDEST, "
CQUERY += "        CASE "
CQUERY += "          WHEN SB1.B1_COD IN(" + cDesova + ") "
CQUERY += "          THEN 'S' "
CQUERY += "          ELSE 'N' "
CQUERY += "        END DESOVA, "
CQUERY += "        NVL(EST.CASA,0) CASA "
CQUERY += "FROM  SB2010 SB2 RIGHT JOIN SB1010 SB1 "
CQUERY += "      ON  SB1.B1_FILIAL  = SB2.B2_FILIAL "
CQUERY += "      AND SB2.B2_COD     = SB1.B1_COD "
CQUERY += "                INNER JOIN (  SELECT SB2.B2_COD,    MAX (SB2.B2_QATU) CASA "
CQUERY += "                              FROM   SB2010 SB2 "
CQUERY += "                              WHERE SB2.B2_FILIAL IN ("+cFilEst+") "
CQUERY += "                              AND SB2.B2_COD IN ( SELECT B1_COD "
CQUERY += "                                                  FROM SB1010 SB1 "
CQUERY += "                                                  WHERE SB1.B1_FILIAL = '05' "
CQUERY += "                                                  AND ( SB1.B1_MSBLQL = '2' OR SB1.B1_MSBLQL    = ' ') "
...
CQUERY += "ORDER BY DESOVA,  QTDEST DESC,  CASA DESC,  B1_EQUIVAL DESC,  B1_SIMILAR DESC,  B1_REFER "
...
CQUERY += "   ( SELECT SB2.B2_COD, "
CQUERY += "     MAX (SB2.B2_QATU) CASA "
CQUERY += "     FROM SB2010 SB2 "
CQUERY += "     WHERE SB2.B2_FILIAL IN ("+cFilEst+") "
CQUERY += "     AND   SB2.B2_COD    IN( SELECT B1_COD FROM SB1010 SB1 "
CQUERY += "                             WHERE SB1.B1_FILIAL   = '05' "
CQUERY += "                             AND ( SB1.B1_MSBLQL   = '2' OR SB1.B1_MSBLQL  = ' ') "
...
CQUERY += "WHERE SB2.B2_FILIAL (+)   = '"+xFilial('SB1')+"' "
CQUERY += "AND   SB2.B2_LOCAL (+)    = '01' "
CQUERY += "AND   SB2.B2_COD (+)      = SB1.B1_COD "
CQUERY += "AND   SB2.D_E_L_E_T_      = ' ' "
CQUERY += "AND   EST.B2_COD (+)      = SB1.B1_COD "
CQUERY += "AND   SB1.B1_FILIAL       = '"+xFilial('SB1')+"' "
...
CQUERY += "ORDER BY DESOVA, "
CQUERY += "  QTDEST DESC, "
CQUERY += "  CASA DESC, "
CQUERY += "  B1_EQUIVAL DESC, "
CQUERY += "  B1_SIMILAR DESC, "
CQUERY += "  B1_REFER   "
*/
