...
Data     : 06/05/11
Descrição: Programação de compras INA-FAG.
/*/
...
User Function PRGINAFAG
	...
	Local   _aSays	     := {}
	Local   _aButtons    := {}
	Local   _cCadastro   := OemtoAnsi("Exportação de Dados - Promoção Goodyear")
	...
	If lViaWorkFlow
		...
		Executar()
	Else
		AADD( _aSays,OemToAnsi("Esta rotina tem por finalidade efetuar a exportação de dados para arquivo de") )
		AADD( _aSays,OemToAnsi("texto, obedecendo layout específico.                                        ") )
...
//*************************************************************************************
// Jean Rehermann - 07/05/2011 - Função principal do programa
//*************************************************************************************
...
_cQuery +=" (SELECT CNPJCLI, PRODUTO, SUM(COMPRADO) COMPRADO, SUM(CONSULTADO) CONSULTADO, SUBSTR( ZA0_DTNECL ,1,6) AS PERIODO FROM ( "
	...
	Iif( Select("PRGQ") <> 0, PRGQ->( dbCloseArea() ), ) // Verifico se a area não está em uso.
	...
	If FTPConnect( GETMV("MV_FTPWEB1"), GETMV("MV_FTPWEB2"), GETMV("MV_FTPWEB3"), GETMV("MV_FTPWEB4") ) // Conectar no FTP
		...
		If FTPDirChange('web/import') // Selecionar o diretório no FTP
			...
			If FTPUpload( '\workflow\PRGINAFAG.txt', 'PRGINAFAG.txt' ) // Fazer upload no arquivo no diretório selecionado no FTP
				CONOUT("PRGINAFAG - ARQUIVO DE PROGRAMAÇÃO ENVIADO COM SUCESSO PARA O PORTAL")
				SEND MAIL FROM _cConta;
				TO GETMV("MV_FTPWEB5");
				SUBJECT 'Portal Imdepa - Arquivo de programação enviado com sucesso';
			...
			Else
				CONOUT("PRGINAFAG - FALHA NO ENVIO DO ARQUIVO DE PROGRAMAÇÃO PARA O PORTAL, SEGUE POR EMAIL")
				SEND MAIL FROM _cConta;
				TO GETMV("MV_FTPWEB5");
				SUBJECT 'Portal Imdepa - Falha no upload do arquivo de programação';
		...
		Else
			...
			BODY 'Não foi possível selecionar o diretório via FTP. O arquivo de programação segue em anexo.';
			ATTACHMENT  '\workflow\PRGINAFAG.txt'
			DISCONNECT SMTP SERVER

		EndIf

		FTPDisconnect() // Desconexão do FTP

	Else
		CONOUT("PRGINAFAG - FALHA NA CONEXAO COM O SERVIDOR DE FTP, SEGUE POR EMAIL")
		SEND MAIL FROM _cConta;
		TO GETMV("MV_FTPWEB5");
		SUBJECT 'Portal Imdepa - Falha na conexão FTP';
		BODY 'Houve falha na conexão como FTP do portal. O arquivo de programação segue em anexo.';
		ATTACHMENT  '\workflow\PRGINAFAG.txt'
		DISCONNECT SMTP SERVER
	...
	If !lViaWorkFlow
		FechaBatch() // Fecha a tela com os botões.
	EndIf
